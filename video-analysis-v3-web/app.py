...
function creatorSearchText(c){{return [c.name,c.kwai_id,c.kwai_url,c.uid,c.phone,c.account_id,c.cooperation_level,c.creator_description,JSON.stringify(c.creator_type||{{}}),JSON.stringify(c.categories||[])].join(" ").toLowerCase()}}
...
function filteredCreators(){{const q=String(document.querySelector("#creator-search")?.value||"").trim().toLowerCase();const scope=String(document.querySelector("#creator-search-scope")?.value||"all");const rows=q?creators.filter(c=>String(creatorFieldValue(c,scope)).toLowerCase().includes(q)):creators;return rows}}
...
function openCreatorModal(){{creatorForm.reset();creatorForm.category.innerHTML=labels.map(x=>`<option value="${{esc(x)}}">${{esc(x)}}</option>`).join("");creatorModal.classList.add("open")}}
function openCreatorTags(id){{const c=creators.find(x=>x.profile_id===id);if(!c)return;const type=c.creator_type||{{}};const account=c.account||{{}};creatorTagsForm.dataset.creatorTags=id;creatorTagsForm.kwai_url.value=c.kwai_url||"";creatorTagsForm.display_name.value=c.name||"";creatorTagsForm.kwai_id.value=c.kwai_id||"";creatorTagsForm.phone.value=c.phone||account.phone||"";creatorTagsForm.uid.value=c.uid||account.uid||"";creatorTagsForm.category.innerHTML=optionList(labels,(c.categories||[])[0]||"","分类待标注");creatorTagsForm.identity.innerHTML=optionList(["夫妻","情侣","家庭","朋友"],type.identity,"身份待标注");creatorTagsForm.location.innerHTML=optionList(["家里","乡村","城市"],type.location,"地点待标注");creatorTagsForm.cooperation_level.innerHTML=optionList(["高","中","低","待观察"],c.cooperation_level,"配合度待标注");creatorTagsForm.creator_description.value=c.creator_description||"";creatorTagsModal.classList.add("open")}}
...
function creatorTile(c){{const account=c.account||{{}};const type=c.creator_type||{{}};const avatar=esc(c.avatar_url||creatorFallbackAvatar());const name=c.name||c.kwai_id||"Kwai creator";const kwai=c.kwai_id||"-";const uid=c.uid||account.uid||"-";const phone=c.phone||account.phone||"-";const fans=creatorMetric(c,["followers","follower_count","fans"],"-");const posts=creatorMetric(c,["post_count","video_count","works_count","total_posts"],"-");return `<article class="creator-tile" data-open-creator="${{esc(c.profile_id)}}"><div class="creator-tile-head"><img class="avatar" src="${{avatar}}" alt=""><div><h3 class="creator-tile-name">${{esc(name)}}</h3><div class="small">Kwai ID&nbsp; ${{esc(kwai)}}</div></div></div><div class="meta"><span class="creator-badge">${{esc(c.cooperation_level||"Normal")}}</span>${{(c.categories||[]).slice(0,2).map(x=>`<span class="pill">${{esc(x)}}</span>`).join("")}}</div><div class="creator-field-lines"><div class="creator-field-line"><span>UID</span><span>${{esc(uid)}}</span></div><div class="creator-field-line"><span>电话</span><span>${{esc(phone)}}</span></div></div><div class="creator-stats-row"><div class="creator-stat"><span>粉丝数</span><b>${{esc(fans)}}</b></div><div class="creator-stat"><span>总发文数</span><b>${{esc(posts)}}</b></div></div><div class="small" style="margin-top:10px">身份：${{esc(type.identity||"待标注")}} · 地点：${{esc(type.location||"待标注")}} · 回传 ${{Number(c.submission_count||0)}}</div></article>`}}
function creatorSubmissionList(c){{const subs=(c.submissions||[]).filter(s=>s.status!=="placeholder");return subs.length?subs.map(s=>`<div class="script-mini"><img src="${{esc(s.thumbnail_url||"")}}" alt=""><div><b>${{esc(s.submitted_title||s.script_title||"回传视频")}}</b><span>脚本：${{esc(s.script_title||s.entry_id||"")}}</span><span>发布者：${{esc(s.detected_kwai_id?"@"+s.detected_kwai_id:(s.creator_id||"未识别"))}} · ${{esc(timeText(s.created_at))}}</span><span>${{esc(s.video_url||"")}}</span></div><div class="actions"><a class="btn" href="${{esc(scriptShareUrl(s.entry_id))}}" target="_blank" rel="noopener">脚本</a><a class="btn" href="${{esc(s.video_url||"#")}}" target="_blank" rel="noopener">视频</a></div></div>`).join(""):`<div class="empty">这个作者还没有回传。</div>`}}
function creatorDetail(c){{const account=c.account||{{}};const type=c.creator_type||{{}};const avatar=esc(c.avatar_url||creatorFallbackAvatar());const name=c.name||c.kwai_id||"Kwai creator";const fans=creatorMetric(c,["followers","follower_count","fans"],"未抓到");const likes=creatorMetric(c,["likes","favorites","like_count"],"未抓到");return `<div class="creator-detail-top"><button type="button" data-back-creators>返回列表</button><div class="actions"><button class="primary" type="button" data-edit-creator-tags="${{esc(c.profile_id)}}">编辑标签</button><button type="button" data-refresh-creator="${{esc(c.profile_id)}}">刷新资料</button><button class="danger" type="button" data-delete-creator="${{esc(c.profile_id)}}">删除</button></div></div><section class="creator-detail-hero"><img class="avatar" src="${{avatar}}" alt=""><div><h2 style="margin:0 0 6px">${{esc(name)}}</h2><div class="small">@${{esc(c.kwai_id||"")}} · UID ${{esc(c.uid||account.uid||"未填")}} · 电话 ${{esc(c.phone||account.phone||"未填")}}</div><a class="small" href="${{esc(c.kwai_url)}}" target="_blank" rel="noopener">${{esc(c.kwai_url||"")}}</a><div class="meta"><span class="pill">配合度 ${{esc(c.cooperation_level||"待标注")}}</span><span class="pill">身份 ${{esc(type.identity||"待标注")}}</span><span class="pill">地点 ${{esc(type.location||"待标注")}}</span>${{(c.categories||[]).map(x=>`<span class="pill">${{esc(x)}}</span>`).join("")}}</div></div><div class="submission-count"><b>${{Number(c.submission_count||0)}}</b><span>回传</span></div></section><div class="creator-detail-grid"><section class="import-panel"><h3 style="margin:0;color:#ff8200">推荐脚本</h3><p class="small">点击后按当前标签现场计算，只展示前 5 个。</p><button type="button" data-load-creator-scripts="${{esc(c.profile_id)}}">查看推荐脚本</button><div id="creator-scripts-${{esc(c.profile_id)}}" class="grid"></div></section><section class="import-panel"><h3 style="margin:0;color:#ff8200">作者数据</h3><div class="meta"><span class="pill">粉丝 ${{esc(fans)}}</span><span class="pill">赞藏 ${{esc(likes)}}</span><span class="pill">覆盖脚本 ${{Number(c.returned_script_count||0)}}</span></div><p class="copy" style="margin-top:10px">${{esc(c.creator_description||"还没有填写具体作者描述。")}}</p></section></div><section class="import-panel"><h3 style="margin:0;color:#ff8200">回传数据</h3>${{creatorSubmissionList(c)}}</section>`}}
function renderCreators(){{const list=document.querySelector("#creator-list");if(!list)return;const rows=filteredCreators();if(selectedCreatorId&&!creators.some(c=>c.profile_id===selectedCreatorId))selectedCreatorId="";const detail=selectedCreatorId?creators.find(c=>c.profile_id===selectedCreatorId):null;if(detail){{list.innerHTML=creatorDetail(detail);const meta=document.querySelector("#creator-results-meta");if(meta)meta.innerHTML=`Total of <b>${{creators.length}}</b> · 当前作者：${{esc(detail.name||detail.kwai_id||detail.profile_id)}}`;const s=document.querySelector("#status");if(s)s.textContent="点击“查看推荐脚本”后才会加载推荐，避免列表卡顿。";return}}const total=rows.length;const pages=Math.max(1,Math.ceil(total/CREATOR_PAGE_SIZE));creatorPage=Math.min(Math.max(1,creatorPage),pages);const start=(creatorPage-1)*CREATOR_PAGE_SIZE;const pageRows=rows.slice(start,start+CREATOR_PAGE_SIZE);const meta=document.querySelector("#creator-results-meta");if(meta)meta.innerHTML=`Total of <b>${{total}}</b> | 所有结果(${{total}}) | kwaiId(${{rows.filter(c=>c.kwai_id).length}}) | 用户名(${{rows.filter(c=>c.name).length}})`;if(!pageRows.length){{list.innerHTML=`<div class="empty">没有匹配创作者。点击“导入创作者”添加一个 Kwai 作者主页。</div>`;return}}list.innerHTML=`<div class="creator-card-grid">${{pageRows.map(creatorTile).join("")}}</div><div class="creator-pagination"><button type="button" data-creator-page="prev" ${{creatorPage<=1?"disabled":""}}>‹</button><span class="pill">${{creatorPage}} / ${{pages}}</span><button type="button" data-creator-page="next" ${{creatorPage>=pages?"disabled":""}}>›</button><span class="small">18/page</span></div>`;const s=document.querySelector("#status");if(s)s.textContent=`已显示第 ${{start+1}}-${{Math.min(start+CREATOR_PAGE_SIZE,total)}} 个，共 ${{total}} 个创作者。`}}
...
async function saveCreatorTags(formEl){{const id=formEl?.dataset?.creatorTags||"";if(!id)return;const fd=new FormData(formEl);const status=document.querySelector("#status");if(status)status.textContent="正在保存作者标签...";const payload={{kwai_url:fd.get("kwai_url"),display_name:fd.get("display_name"),kwai_id:fd.get("kwai_id"),phone:fd.get("phone"),uid:fd.get("uid"),categories:fd.get("category")?[fd.get("category")]:[],identity:fd.get("identity"),location:fd.get("location"),cooperation_level:fd.get("cooperation_level"),creator_description:fd.get("creator_description"),skip_fetch:true}};try{{const d=await api(`/api/creator-admin/creators/${{id}}`,{{method:"POST",body:JSON.stringify(payload)}});const updated=d.creator;if(updated){{const index=creators.findIndex(x=>x.profile_id===id);if(index>=0)creators[index]=updated}}creatorTagsModal.classList.remove("open");renderCreators();if(status)status.textContent="作者标签已保存。点击“查看推荐脚本”会按新标签重新计算推荐。"}}catch(err){{if(status)status.textContent=err.message}}}}
async function loadCreatorScripts(id){{const box=document.getElementById(`creator-scripts-${{id}}`);if(!box)return;box.innerHTML=`<div class="empty">正在按标签计算推荐脚本...</div>`;try{{const d=await api(`/api/creator-admin/creators/${{id}}/recommendations?limit=5`);const scripts=d.scripts||[];box.innerHTML=scripts.length?scripts.slice(0,5).map(scriptMini).join(""):`<div class="empty">暂时没有匹配脚本。请先调整作者分类标签后重试。</div>`;const status=document.querySelector("#status");if(status)status.textContent=`已为该作者加载 ${{scripts.length}} / ${{Number(d.total||scripts.length||0)}} 个推荐脚本。`;}}catch(err){{box.innerHTML=`<div class="empty">${{esc(err.message||err)}}</div>`}}}}
...
async function createCreator(e){{e.preventDefault();const fd=new FormData(e.target);const status=document.querySelector("#status");if(status)status.textContent="正在保存创作者并绑定账号...";try{{await api("/api/creator-admin/creators",{{method:"POST",body:JSON.stringify({{kwai_url:fd.get("kwai_url"),display_name:fd.get("display_name"),kwai_id:fd.get("kwai_id"),phone:fd.get("phone"),uid:fd.get("uid"),categories:fd.get("category")?[fd.get("category")]:[],identity:fd.get("identity"),location:fd.get("location"),cooperation_level:fd.get("cooperation_level"),creator_description:fd.get("creator_description")}})}});e.target.reset();creatorModal.classList.remove("open");selectedCreatorId="";await loadCreators()}}catch(err){{if(status)status.textContent=err.message}}}}
