...
function creatorTile(c){{const account=c.account||{{}};const avatar=esc(c.avatar_url||creatorFallbackAvatar());const name=c.name||c.kwai_id||"Kwai creator";const kwai=c.kwai_id||"-";const uid=c.uid||account.uid||"-";const phone=c.phone||account.phone||"-";const poc=creatorPocValue(c)||"未分配";const source=c.creator_source==="registered_phone_only"?"待绑定 Kwai":(c.registration_status==="registered"?"已注册":"POC 档案");return `<article class="creator-tile" data-open-creator="${{esc(c.profile_id)}}"><div class="creator-tile-head"><img class="avatar" src="${{avatar}}" alt=""><div><h3 class="creator-tile-name">${{esc(name)}}</h3><div class="small">Kwai ID&nbsp; ${{esc(kwai)}}</div></div></div><div class="meta"><span class="creator-badge">${{esc(source)}}</span><span class="pill">POC ${{esc(poc)}}</span>${{(c.categories||[]).slice(0,2).map(x=>`<span class="pill">${{esc(x)}}</span>`).join("")}}</div><div class="creator-field-lines"><div class="creator-field-line"><span>UID</span><span>${{esc(uid)}}</span></div><div class="creator-field-line"><span>电话</span><span>${{esc(phone)}}</span></div></div><div class="creator-stats-row"><div class="creator-stat"><span>投喂脚本</span><b>${{creatorFeedCount(c)}}</b></div><div class="creator-stat"><span>回传脚本</span><b>${{creatorFeedReturnCount(c)}}</b></div><div class="creator-stat"><span>打开 Koko</span><b>${{creatorOpenCount(c)}}</b></div><div class="creator-stat"><span>打开脚本</span><b>${{creatorOpenedScriptCount(c)}}</b></div></div></article>`}}
...
function creatorDailyFeedItems(rows){{const out=[];const seen=new Set();for(const c of (rows||creatorDisplayedRows())){{if(creatorIsTestRow(c))continue;const creatorName=String(c.name||c.kwai_id||c.phone||c.account_id||"未命名作者");for(const feed of creatorCombinedFeeds(c)){{if(!feed||!feed.entry_id)continue;const day=String(feed.date||feed.feed_time||"").slice(0,10);if(!day)continue;const key=[c.profile_id||creatorName,feed.entry_id,day,feed.return_url||""].join("|");if(seen.has(key))continue;seen.add(key);out.push({{day,creator_name:creatorName,entry_id:feed.entry_id,script_title:creatorDailyEntryTitle(feed.entry_id),share_url:creatorDailyScriptUrl(feed.entry_id,feed.share_url),feed_time:feed.feed_time||feed.date||day,status:feed.status||"未完成",return_url:feed.return_url||"",return_time:feed.return_time||""}})}}}}return out}}
function creatorDailyReturnItems(rows){{return creatorDailyFeedItems(rows).filter(item=>item.return_url||String(item.status||"").trim()==="已回传").map(item=>({{...item,day:item.return_time?analyticsDayKey(item.return_time):item.day}}))}}
...
function creatorMonthKey(day){{return String(day||"").slice(0,7)}}
function creatorWeekInfo(day){{const d=new Date(`${{day}}T00:00:00+08:00`);if(Number.isNaN(d.getTime()))return {{key:`${{day}}-week`,label:"未记录周"}};const offset=(d.getDay()+6)%7;const start=new Date(d);start.setDate(d.getDate()-offset);const end=new Date(start);end.setDate(start.getDate()+6);const ymdLocal=x=>x.toLocaleDateString("sv-SE",{{timeZone:"Asia/Shanghai"}});return {{key:ymdLocal(start),label:`${{ymdLocal(start).replaceAll("-",".")}} - ${{ymdLocal(end).replaceAll("-",".")}}`}}}}
...
function creatorDailyItemHtml(day,bucket){{const uses=bucket.uses.slice().sort((a,b)=>String(a.time||"").localeCompare(String(b.time||"")));const feeds=bucket.feeds.slice().sort((a,b)=>String(a.feed_time||"").localeCompare(String(b.feed_time||"")));const returns=bucket.returns.slice().sort((a,b)=>String(a.return_time||"").localeCompare(String(b.return_time||"")));return `<details class="creator-day-detail"><summary><b>${{esc(day.replaceAll("-","."))}}</b><span>使用 ${{esc(creatorUsageText(day,bucket))}} 人</span><span>投喂 ${{feeds.length}} 个</span><span>回传 ${{returns.length}} 个</span></summary><div class="creator-day-sections">${{creatorDailySection("当日使用 Koko 的人",creatorUsageDayIsClean(day)?uses:[],item=>`<div class="creator-day-item"><b>${{esc(item.person_name)}}</b><small>${{esc(timeText(item.time))}} · ${{esc(item.source||item.path||"打开脚本")}}</small><small>脚本：${{esc(item.script_title)}}</small></div>`,creatorUsageDayIsClean(day)?"当天没有脚本打开记录。":"8月7日之前的打开数据不计入，用 / 表示。")}}${{creatorDailySection("当日投喂脚本",feeds,item=>`<div class="creator-day-item"><b>${{esc(item.creator_name)}}</b><small>投喂时间：${{esc(item.feed_time||item.day)}}</small><a href="${{esc(item.share_url)}}" target="_blank" rel="noopener">脚本：${{esc(item.script_title)}}</a></div>`,"当天没有投喂记录。")}}${{creatorDailySection("当日回传脚本",returns,item=>`<div class="creator-day-item"><b>${{esc(item.creator_name)}}</b><small>回传时间：${{esc(item.return_time?timeText(item.return_time):item.day)}}</small><a href="${{esc(item.share_url)}}" target="_blank" rel="noopener">脚本：${{esc(item.script_title)}}</a>${{item.return_url?`<a href="${{esc(item.return_url)}}" target="_blank" rel="noopener">视频：${{esc(item.return_url)}}</a>`:""}}</div>`,"当天没有回传记录。")}}</div></details>`}}
...
function creatorDailyDetailsHtml(metricRows){{ensureCreatorDailyStyles();const map=new Map();for(const item of creatorDailyUseItems()){{if(!creatorUsageDayIsClean(item.day))continue;const bucket=creatorDailyBucket(map,item.day);bucket.day=item.day;bucket.uses.push(item);if(item.person_key)bucket.users.set(item.person_key,item)}}for(const item of creatorDailyFeedItems(metricRows)){{const bucket=creatorDailyBucket(map,item.day);bucket.day=item.day;bucket.feeds.push(item)}}for(const item of creatorDailyReturnItems(metricRows)){{const bucket=creatorDailyBucket(map,item.day);bucket.day=item.day;bucket.returns.push(item)}}const days=[...map.entries()].filter(([day,b])=>day&&day!=="未记录日期"&&(b.users.size||b.feeds.length||b.returns.length)).sort((a,b)=>String(b[0]).localeCompare(String(a[0])));if(!days.length)return `<details class="creator-daily-detail"><summary><span>查看具体数据</span><small>暂无脚本打开、投喂或回传明细；8月7日前打开数据用 / 表示</small></summary><div class="creator-daily-list"><div class="empty">目前还没有可展开的每日数据。</div></div></details>`;const months=new Map();for(const item of days){{const month=creatorMonthKey(item[0]);if(!months.has(month))months.set(month,[]);months.get(month).push(item)}}return `<details class="creator-daily-detail"><summary><span>查看具体数据</span><small>先看月统计，再展开到周和日；8月7日前打开数据不计入</small></summary><div class="creator-daily-list">${{[...months.entries()].sort((a,b)=>String(b[0]).localeCompare(String(a[0]))).map(([month,monthDays])=>{{const monthCounts=creatorPeriodCounts(monthDays);const weeks=new Map();for(const item of monthDays){{const info=creatorWeekInfo(item[0]);if(!weeks.has(info.key))weeks.set(info.key,{{label:info.label,days:[]}});weeks.get(info.key).days.push(item)}}return `<details class="creator-day-detail creator-month-detail"><summary><b>${{esc(month.replace("-","."))}}</b><span>使用 ${{esc(monthCounts.usage)}} 人</span><span>投喂 ${{monthCounts.feeds}} 个</span><span>回传 ${{monthCounts.returns}} 个</span></summary><div class="creator-daily-list">${{[...weeks.entries()].sort((a,b)=>String(b[0]).localeCompare(String(a[0]))).map(([weekKey,week])=>{{const weekCounts=creatorPeriodCounts(week.days);return `<details class="creator-day-detail creator-week-detail"><summary><b>${{esc(week.label)}}</b><span>使用 ${{esc(weekCounts.usage)}} 人</span><span>投喂 ${{weekCounts.feeds}} 个</span><span>回传 ${{weekCounts.returns}} 个</span></summary><div class="creator-daily-list">${{week.days.map(([day,bucket])=>creatorDailyItemHtml(day,bucket)).join("")}}</div></details>`}}).join("")}}</div></details>`}}).join("")}}</div></details>`}}
...
const feedDay=String(feed.date||feed.feed_time||"").slice(0,10);
if(feedDay){{
const item={{day:feedDay,poc,creator_name:creatorName,entry_id:feed.entry_id,script_title:creatorDailyEntryTitle(feed.entry_id),share_url:creatorDailyScriptUrl(feed.entry_id,feed.share_url),feed_time:feed.feed_time||feed.date||feedDay}};
...
const returnDay=feed.return_time?analyticsDayKey(feed.return_time):feedReturnDate(feed);
if(returnDay){{
...
function creatorPocDayHtml(day,bucket){{const feeds=bucket.feeds.slice().sort((a,b)=>String(a.creator_name).localeCompare(String(b.creator_name))||String(a.script_title).localeCompare(String(b.script_title)));const returns=bucket.returns.slice().sort((a,b)=>String(a.creator_name).localeCompare(String(b.creator_name))||String(a.return_time||"").localeCompare(String(b.return_time||"")));return `<details class="creator-day-detail"><summary><b>${{esc(day.replaceAll("-","."))}}</b><span>投喂 ${{feeds.length}} 个</span><span>覆盖 ${{bucket.feedCreators.size}} 人</span><span>回传 ${{returns.length}} 个</span></summary><div class="creator-day-sections">${{creatorDailySection("投给谁",feeds,item=>`<div class="creator-day-item"><b>${{esc(item.creator_name)}}</b><small>投喂时间：${{esc(item.feed_time||item.day)}}</small><a href="${{esc(item.share_url)}}" target="_blank" rel="noopener">脚本：${{esc(item.script_title)}}</a></div>`,"当天没有投喂记录。")}}<section class="creator-day-section"><h4>作者投喂数量</h4><div class="meta">${{creatorPocPersonCounts(bucket.feedCreators)}}</div></section>${{creatorDailySection("收回视频",returns,item=>`<div class="creator-day-item"><b>${{esc(item.creator_name)}}</b><small>回传时间：${{esc(item.return_time?timeText(item.return_time):item.day)}}</small><a href="${{esc(item.share_url)}}" target="_blank" rel="noopener">脚本：${{esc(item.script_title)}}</a>${{item.return_url?`<a href="${{esc(item.return_url)}}" target="_blank" rel="noopener">视频：${{esc(item.return_url)}}</a>`:""}}</div>`,"当天没有回传记录。")}}</div></details>`}}
function creatorPocPanelHtml(rows,poc){{const dayMap=creatorPocDailyMap(rows,poc);const days=[...dayMap.entries()].filter(([,b])=>b.feeds.length||b.returns.length).sort((a,b)=>String(b[0]).localeCompare(String(a[0])));const today=creatorLocalDay();const todayBucket=dayMap.get(today)||{{feeds:[],returns:[],feedCreators:new Map(),returnCreators:new Map()}};const weekKey=creatorWeekInfo(today).key;const monthKey=creatorMonthKey(today);const weekDays=days.filter(([day])=>creatorWeekInfo(day).key===weekKey);const monthDays=days.filter(([day])=>creatorMonthKey(day)===monthKey);const weekCounts=creatorPocPeriodCounts(weekDays);const monthCounts=creatorPocPeriodCounts(monthDays);if(!days.length)return `<details class="creator-day-detail"><summary><b>${{esc(poc)}}</b><span>今日投喂 0</span><span>今日回传 0</span><span>暂无记录</span></summary><div class="empty">这个 POC 还没有投喂或回传记录。</div></details>`;const months=new Map();for(const item of days){{const month=creatorMonthKey(item[0]);if(!months.has(month))months.set(month,[]);months.get(month).push(item)}}return `<details class="creator-day-detail creator-poc-detail"><summary><b>${{esc(poc)}}</b><span>今日投喂 ${{todayBucket.feeds.length}} / 回传 ${{todayBucket.returns.length}}</span><span>本周 ${{weekCounts.feeds}} / ${{weekCounts.returns}}</span><span>本月 ${{monthCounts.feeds}} / ${{monthCounts.returns}}</span></summary><div class="creator-daily-list">${{[...months.entries()].sort((a,b)=>String(b[0]).localeCompare(String(a[0]))).map(([month,monthDays])=>{{const monthCounts=creatorPocPeriodCounts(monthDays);const weeks=new Map();for(const item of monthDays){{const info=creatorWeekInfo(item[0]);if(!weeks.has(info.key))weeks.set(info.key,{{label:info.label,days:[]}});weeks.get(info.key).days.push(item)}}return `<details class="creator-day-detail creator-month-detail"><summary><b>${{esc(month.replace("-","."))}}</b><span>投喂 ${{monthCounts.feeds}} 个</span><span>覆盖 ${{monthCounts.feedCreators}} 人</span><span>回传 ${{monthCounts.returns}} 个</span></summary><div class="creator-daily-list">${{[...weeks.entries()].sort((a,b)=>String(b[0]).localeCompare(String(a[0]))).map(([weekKey,week])=>{{const weekCounts=creatorPocPeriodCounts(week.days);return `<details class="creator-day-detail creator-week-detail"><summary><b>${{esc(week.label)}}</b><span>投喂 ${{weekCounts.feeds}} 个</span><span>覆盖 ${{weekCounts.feedCreators}} 人</span><span>回传 ${{weekCounts.returns}} 个</span></summary><div class="creator-daily-list">${{week.days.map(([day,bucket])=>creatorPocDayHtml(day,bucket)).join("")}}</div></details>`}}).join("")}}</div></details>`}}).join("")}}</div></details>`}}
function creatorPocStatsHtml(metricRows){{return `<details class="creator-daily-detail creator-poc-stats"><summary><span>POC 投喂/回收统计</span><small>单独查看 denghaoqing 和 zhaozhe 每日、每周、每月投喂与回传</small></summary><div class="creator-daily-list">${{creatorPocOptions.map(poc=>creatorPocPanelHtml(metricRows,poc)).join("")}}</div></details>`}}
function creatorOverviewHtml(){{const rows=creatorDisplayedRows();const registered=accounts.filter(a=>creatorAccountIsRegistered(a)&&!analyticsIsTestUser(a));const registeredKeys=new Set(registered.map(a=>String(a.account_id||a.phone||"")));const metricRows=rows.filter(c=>!creatorIsTestRow(c)&&(c.registration_status==="registered"||registeredKeys.has(String(c.account_id||c.phone||""))||creatorFeedReturnCount(c)>0||creatorFeedCount(c)>0));const scriptCount=metricRows.reduce((sum,c)=>sum+creatorOpenedScriptCount(c),0);const returnCount=metricRows.reduce((sum,c)=>sum+creatorFeedReturnCount(c),0);return `<section class="analytics-kpis analytics-core-kpis" style="margin:0 0 16px">${{analyticsSummaryCard("注册用户",registered.length,"已激活或自助注册，已排除测试账号 666")}}${{analyticsSummaryCard("打开脚本个数",scriptCount,"注册用户 + 有投喂回收的明确作者，已排除 666")}}${{analyticsSummaryCard("回传数",returnCount,"按投喂脚本及回收情况统计，已排除测试账号 666")}}</section>${{creatorDailyDetailsHtml(metricRows)}}${{creatorPocStatsHtml(metricRows)}}`}}
...
async function confirmCreatorRecommendFeeds(id){{const root=document.getElementById(`creator-scripts-${{id}}`);const cards=[...(root?.querySelectorAll("[data-recommend-selected] [data-recommend-card]")||[])];const status=document.querySelector("#status");if(!cards.length){{if(status)status.textContent="请先把要投喂的脚本拖到右侧。";return}}const lines=cards.map((card,idx)=>`${{idx+1}}. ${{card.dataset.share||scriptShareUrl(card.dataset.entryId)}}`);await copyTextValue(lines.join("\\n"));const today=feedDate();const next=[...creatorFeeds(id),...cards.map(card=>feedNormalize({{entry_id:card.dataset.entryId,share_url:card.dataset.share,date:today,status:"未完成"}})).filter(Boolean)];const seen=new Set();const dedup=next.filter(feed=>{{const key=`${{feed.entry_id}}:${{feed.date}}`;if(seen.has(key))return false;seen.add(key);return true}});setCreatorState(id,{{feeds:dedup}});await saveCreatorCloudState(id,{{feeds:dedup}});await hydrateCreatorFeeds(id,false);let box=root.querySelector(".creator-recommend-copy-box");if(!box){{box=document.createElement("div");box.className="creator-recommend-copy-box";root.appendChild(box)}}box.textContent=`已复制并记录到 ${{today}}：\\n${{lines.join("\\n")}}`;if(status)status.textContent=`已复制 ${{cards.length}} 条分享链接，并保存为 ${{today}} 的投喂记录。`}}
