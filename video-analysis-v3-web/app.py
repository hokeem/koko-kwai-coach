...
function renderAnalytics(){const box=document.querySelector("#analytics-board");if(!box)return;if(!analyticsData){box.innerHTML=`<div class="empty">正在自动加载有行为用户。无行为账号会保持折叠，展开时再加载。</div>`;return}const s=analyticsData.summary||{};const users=analyticsData.users||[];const inactive=analyticsData.inactive_users||[];const inactiveTotal=Number(s.inactive_accounts||inactive.length||0);const registeredUsers=users.filter(u=>analyticsIsRegistered(u));const registeredDurationSeconds=registeredUsers.reduce((sum,u)=>sum+Number(u.platform_duration_seconds||0)+Number(u.script_duration_seconds||0),0);const avgRegisteredStaySeconds=registeredUsers.length?registeredDurationSeconds/registeredUsers.length:0;const submissionCount=Number(s.submissions||users.reduce((sum,u)=>sum+Number(u.submission_count||0),0));const kpis=[["已注册用户数量",registeredUsers.length,`按已注册账号和首次回传账号统计`],["注册用户平均停留时长",secondsText(avgRegisteredStaySeconds),`注册用户总停留 ${secondsText(registeredDurationSeconds)}`],["回传数量",submissionCount,`作者提交的视频外链总数`]];box.innerHTML=`<div class="analytics-shell"><section class="analytics-kpis analytics-core-kpis">${kpis.map(([label,value,sub])=>analyticsSummaryCard(label,value,sub)).join("")}</section>${analyticsCoreTimelineHtml()}<section class="analytics-layout"><div class="analytics-panel"><div class="analytics-panel-title"><h3>作者状态</h3>${analyticsFilterTabs()}</div>${analyticsAuthorRows(users)}</div></section>${analyticsInactiveRows(inactive,inactiveTotal,analyticsData.inactive_loaded)}</div>`;wireAnalyticsControls();const status=document.querySelector("#status");if(status)status.textContent=`核心数据已加载：已注册用户 ${registeredUsers.length} 个，平均停留 ${secondsText(avgRegisteredStaySeconds)}，回传 ${submissionCount} 条。无行为账号 ${inactiveTotal} 个已折叠。`}
