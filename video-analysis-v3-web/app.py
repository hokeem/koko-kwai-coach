...
async function loadEntries(){{try{{document.querySelector("#status")&&(document.querySelector("#status").textContent="加载中...");const d=await api("/api/creator-admin/scripts");entries=d.entries||[];if(activeScriptType&&!entries.some(e=>String(e.content_type||"待分类").trim()===activeScriptType))activeScriptType="";adminView();document.querySelector("#status").textContent=`共 ${{entries.length}} 条 Creator 脚本`}}catch(e){{loginView(e.message)}}}}
...
async function loadCurrentTab(){{if(activeTab==="creators")return loadCreators();if(activeTab==="submissions")return loadSubmissions();if(activeTab==="intakes")return loadIntakes();if(activeTab==="imports"){{adminView();return}}return loadEntries()}}
