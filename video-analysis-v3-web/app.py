...
CREATOR_ADMIN_ANALYTICS_SUMMARY_CACHE_FILE = DATA_ROOT / "creator_admin_analytics_summary_cache.json"
...
CREATOR_ADMIN_PASSWORD = os.environ.get("KOKO_CREATOR_ADMIN_PASSWORD", "kokokwai@2026")
...
def apply_script_taxonomy_classification(
    ...
    *,
    ...
    manual_dimensions = (
        set(SCRIPT_TAG_DIMENSIONS)
        if str(entry.get("taxonomy_source") or "").strip().lower() == "manual"
        else set()
    )
    classified = (
...
def apply_manual_item_taxonomy(parent_job_id: str, item_index: int, payload: dict[str, Any]) -> None:
    if not isinstance(payload, dict):
        return
    taxonomy_fields = {f"{dimension}_tags" for dimension in SCRIPT_TAG_DIMENSIONS}
    if not taxonomy_fields.intersection(payload) and "duration_bucket" not in payload:
        return
    taxonomy_tags, errors = parse_taxonomy_tags_payload(payload)
    if errors:
        raise RuntimeError("Unsupported taxonomy tags: " + "; ".join(errors))
    duration_bucket = str(payload.get("duration_bucket") or "").strip()
    if duration_bucket and duration_bucket not in taxonomy_option_map("duration"):
        raise RuntimeError("Unsupported duration bucket.")
    changes: dict[str, Any] = {
        "taxonomy_version": SCRIPT_TAXONOMY_VERSION,
        "taxonomy_source": "manual",
        "taxonomy_confidence": "manual",
        "taxonomy_updated_at": now_iso(),
    }
    for dimension in SCRIPT_TAG_DIMENSIONS:
        changes[f"{dimension}_tags"] = taxonomy_tags[dimension]
    if duration_bucket:
  ...
  <script>
    ...
    async function loadCreatorAnalytics() {{
      ...
      try {{
        ...
        if (root) root.innerHTML = `<div class="stats-empty">${{escStats(err.message || err)}}<br>如果你还没登录 Creator 运营后台，请先打开 /koko_admin_2026-kwai 登录一次。</div>`;
      }}
    }}
...
def public_item_view(item: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "taxonomy_version": item.get("taxonomy_version") or SCRIPT_TAXONOMY_VERSION,
        "taxonomy_source": item.get("taxonomy_source") or "auto",
        "taxonomy_confidence": item.get("taxonomy_confidence") or "",
        **{
            f"{dimension}_tags": normalize_taxonomy_tag_ids(dimension, item.get(f"{dimension}_tags"))
            for dimension in SCRIPT_TAG_DIMENSIONS
        },
...
def build_library_entry_payload(parent_job_id: str, item: dict[str, Any], *, use_llm: bool = True) -> dict[str, Any]:
    ...
    entry = {
        ...
        **duration_fields,
        ...
        "duration_bucket": item.get("duration_bucket") or existing.get("duration_bucket") or duration_fields.get("duration_bucket") or "",
        **{
    ...
    <div class="hero-panel">
      <div class="brandbar">
        <div class="navpill">
          ...
          <a href="/koko_admin_2026-kwai">Creator Ops</a>
...
<body>
  <main class="studio-shell">
    <aside class="studio-sidebar">
      ...
      <nav class="studio-side-nav" aria-label="Koko 内容中台导航">
        ...
        <a class="studio-tab-link" href="/koko_admin_2026-kwai"><span class="studio-tab-icon">★</span><span>Creator 运营</span></a>
      </nav>
  ...
  <script>
    ...
    const SCRIPT_TAXONOMY = {json.dumps({
    ...
    const TAXONOMY_DIMENSION_META = {{ relationship: "人物关系", format: "形式", location: "地点", content: "内容", duration: "时间" }};

    function itemTaxonomyIds(item, dimension) {{
      if (dimension === "duration") return item.duration_bucket ? [String(item.duration_bucket)] : [];
      const values = item[`${{dimension}}_tags`];
      return Array.isArray(values) ? values.map(String) : [];
    }}

    function manualTagPickerMarkup(item, compact = false) {{
      const rows = Object.entries(TAXONOMY_DIMENSION_META).map(([dimension, label]) => {{
        const selected = new Set(itemTaxonomyIds(item, dimension));
        const options = (SCRIPT_TAXONOMY.dimensions[dimension] || []).map((option) => `
          <button class="manual-taxonomy-chip ${{selected.has(String(option.id)) ? "active" : ""}}" type="button"
            data-manual-taxonomy="${{dimension}}" data-taxonomy-id="${{escapeHtml(option.id)}}">
            ${{escapeHtml(option.zh)}}
          </button>
        `).join("");
        return `<div class="manual-taxonomy-row"><span class="manual-taxonomy-label">${{label}}</span><div class="manual-taxonomy-options">${{options}}</div></div>`;
      }}).join("");
      return `
        <div class="manual-tag-picker" data-manual-taxonomy-picker="${{item.id}}">
          <div class="manual-tag-heading">${{compact ? "人工修改入库标签" : "确认或修改脚本标签"}}</div>
          ${{rows}}
          <div class="manual-taxonomy-feedback" data-taxonomy-feedback role="status">人物关系、形式、地点和内容可多选；时间只能选一项。</div>
        </div>
      `;
    }}

    function selectedManualTaxonomy(itemId, root = document) {{
      const picker = root.querySelector(`[data-manual-taxonomy-picker="${{itemId}}"]`) || document.querySelector(`[data-manual-taxonomy-picker="${{itemId}}"]`);
      const payload = {{ taxonomy_version: SCRIPT_TAXONOMY.version, taxonomy_source: "manual" }};
      Object.keys(TAXONOMY_DIMENSION_META).forEach((dimension) => {{
        const selected = picker ? Array.from(picker.querySelectorAll(`[data-manual-taxonomy="${{dimension}}"].active`)).map((button) => button.dataset.taxonomyId) : [];
        if (dimension === "duration") payload.duration_bucket = selected[0] || "";
        else payload[`${{dimension}}_tags`] = selected;
      }});
      return payload;
    }}
    ...
    function setTaxonomyFeedback(scope, message, kind = "") {{
      ...
      feedback.className = `manual-taxonomy-feedback ${{kind}}`.trim();
    }}
    ...
    function collectItemEdits(itemId) {{
      ...
      if (root.getAttribute("data-editor-mode") === "structured") {{
        ...
        const payload = {{
          ...
          ...selectedManualTaxonomy(itemId, root),
    ...
    async function persistItemEdits(itemId, mode, button) {{
      ...
      setTaxonomyFeedback(scope, "正在保存脚本和新标签...");
      try {{
        ...
        setTaxonomyFeedback(scope, "标签已保存并同步到脚本库。", "success");
        if (activeJobId) {{
          pollJob(activeJobId);
        }} else {{
        ...
        setTaxonomyFeedback(scope, `保存失败：${{String(error.message || error)}}`, "error");
    ...
    async function confirmLibraryEntry(itemId, button) {{
      ...
      try {{
        ...
        setTaxonomyFeedback(scope, "入库成功，新标签已生效。", "success");
        ...
        setTaxonomyFeedback(scope, `入库失败：${{String(error.message || error)}}`, "error");
        button.disabled = false;
        button.textContent = original;
      }}
    }}
    ...
    document.addEventListener("click", (event) => {{
      ...
      if (taxonomyChip) {{
        event.preventDefault();
        const picker = taxonomyChip.closest("[data-manual-taxonomy-picker]");
        const dimension = taxonomyChip.dataset.manualTaxonomy || "";
        if (dimension === "duration") {{
          picker?.querySelectorAll('[data-manual-taxonomy="duration"]').forEach((button) => button.classList.remove("active"));
          taxonomyChip.classList.add("active");
        }} else {{
...
CREATOR_ADMIN_TAB_ROUTES = {
    "/koko_admin_2026-kwai": "creators",
    "/koko_admin_2026-kwai/imports": "creators",
    "/koko_admin_2026-kwai/creators": "creators",
    "/koko_admin_2026-kwai/accounts": "creators",
    "/koko_admin_2026-kwai/analytics": "analytics",
    "/koko_admin_2026-kwai/submissions": "submissions",
    "/koko_admin_2026-kwai/intakes": "intakes",
}


def creator_admin_tab_for_path(path: str) -> str | None:
    if re.fullmatch(r"/koko_admin_2026-kwai/creators/[0-9a-f]{32}", path):
        return "creators"
...
function analyticsIsTestUser(item){{return ["account_id","phone","kwai_id","visitor_id","display_name","uid"].some(field=>String(item?.[field]||"").trim()==="666")||["666"].includes(analyticsEntityKey(item))}}
...
function creatorAnalyticsUsers(){{return Array.isArray(analyticsData?.users)?analyticsData.users:[]}}
function creatorAnalyticsScriptOpenTotal(fallbackRows=[]){{const users=creatorAnalyticsUsers().filter(u=>!analyticsIsTestUser(u));const userTotal=users.reduce((sum,u)=>sum+Number(u.script_share_open_count||0),0);if(userTotal)return userTotal;const summaryTotal=Number(analyticsData?.summary?.script_opens||0);if(summaryTotal)return summaryTotal;return (fallbackRows||[]).filter(c=>!creatorIsTestRow(c)).reduce((sum,c)=>sum+creatorOpenedScriptCount(c),0)}}
...
function openCreatorModal(){{creatorForm.reset();const modalStatus=document.querySelector("#creator-form-status");if(modalStatus)modalStatus.textContent="";creatorForm.category.innerHTML=labels.map(x=>`<option value="${{esc(x)}}">${{esc(x)}}</option>`).join("");creatorForm.poc.innerHTML=optionList(creatorPocOptions,"","POC 待分配");creatorForm.identity.innerHTML=multiOptionList(["夫妻","情侣","家庭","朋友"],[]);creatorForm.location.innerHTML=multiOptionList(["家里","乡村","城市"],[]);creatorModal.classList.add("open")}}
...
function selectCreator(id){{selectedCreatorId=id;history.pushState({{creator:id}},"",`/koko_admin_2026-kwai/creators/${{id}}`);renderCreators()}}
function backToCreatorList(){{selectedCreatorId="";history.pushState({{}},"","/koko_admin_2026-kwai/creators");renderCreators()}}
function creatorFeedSortTime(feed,c){{const ret=feedReturn(c,feed?.entry_id);const raw=String(feed?.return_time||ret?.created_at||feed?.date||feed?.feed_time||"").trim();if(!raw)return 0;const parsed=Date.parse(raw.replace(/\//g,"-"));return Number.isFinite(parsed)?parsed:0}}
function creatorSortFeeds(feeds,c){{return [...(feeds||[])].sort((a,b)=>creatorFeedSortTime(b,c)-creatorFeedSortTime(a,c)||String(b.return_time||b.date||b.feed_time||"").localeCompare(String(a.return_time||a.date||a.feed_time||""))||String(a.entry_id||"").localeCompare(String(b.entry_id||"")))}}
function creatorCombinedFeeds(c){{const id=c.profile_id;const feeds=visibleCreatorFeeds(id,[...creatorFeeds(id),...feedsFromSubmissions(c)].filter((feed,idx,arr)=>arr.findIndex(x=>x.entry_id===feed.entry_id)===idx));return creatorSortFeeds(feeds,c)}}
...
function creatorAnalyticsIdentityKeys(item){{const keys=new Set();for(const field of ["visitor_id","account_id","phone","kwai_id","uid"]){{const value=creatorKeyValue(item?.[field]);if(value)keys.add(value)}}for(const alias of (item?.login_aliases||[])){{const value=creatorKeyValue(alias);if(value)keys.add(value)}}return keys}}
function creatorDailyMatchedCreator(item){{const itemKeys=creatorAnalyticsIdentityKeys(item);if(!itemKeys.size)return null;return creatorDisplayedRows().find(c=>{{const sources=[c,c?.account||{{}},c?.analytics_user||{{}}];return sources.some(source=>[...creatorAnalyticsIdentityKeys(source)].some(key=>itemKeys.has(key)))}})||null}}
function creatorAnonymousLabel(item){{const raw=String(item?.visitor_id||item?.account_id||"").trim();return /^[0-9a-f]{{32}}$/i.test(raw)?`未绑定访客（${{raw.slice(0,6)}}…）`:"匿名访客"}}
function creatorDailyPersonName(item){{const matched=creatorDailyMatchedCreator(item);if(matched)return String(matched.name||matched.kwai_id||matched.phone||matched.account_id||"已绑定作者");const direct=String(item?.display_name||item?.name||item?.kwai_id||item?.phone||"").trim();return direct||creatorAnonymousLabel(item)}}
...
function creatorDailyUseItems(){{const out=[];const seen=new Set();const pushUse=(item,dayValue,timeValue,scriptId="",title="")=>{{if(analyticsIsTestUser(item))return;const day=analyticsDayKey(dayValue||timeValue);if(!day||day==="未记录日期")return;const cleanScriptId=String(scriptId||item.script_id||"").trim();const key=analyticsEntityKey(item)||String(item.visitor_id||item.display_name||item.phone||item.account_id||item.kwai_id||"").trim()||creatorDailyPersonName(item);const uniq=[day,key,cleanScriptId,title||item.script_title||"",timeValue||item.time||""].join("|");if(seen.has(uniq))return;seen.add(uniq);out.push({{...item,day,person_key:key,person_name:creatorDailyPersonName(item),script_id:cleanScriptId,script_title:creatorDailyEntryTitle(cleanScriptId,title||item.script_title),time:timeValue||item.time||day}})}};const hourRows=analyticsData?.timeline?.hourly||[];for(const row of hourRows){{for(const item of (row.script_open_details||[]))pushUse(item,item.time||row.hour,item.time||row.hour,item.script_id,item.script_title)}}for(const user of creatorAnalyticsUsers()){{if(analyticsIsTestUser(user))continue;const userTime=user.last_event_at||user.last_login_at||analyticsActivationTime(user)||user.created_at;const views=Array.isArray(user.script_views)?user.script_views:[];for(const view of views)pushUse({{...user,...view,account_id:user.account_id,phone:user.phone,kwai_id:user.kwai_id,display_name:user.display_name||view.display_name}},view.last_viewed_at||view.created_at||userTime,view.last_viewed_at||view.created_at||userTime,view.script_id,view.title||view.script_title);if(!views.length&&Number(user.script_share_open_count||0)>0)pushUse(user,userTime,userTime,"",`打开脚本 ${{Number(user.script_share_open_count||0)}} 次`)}}if(!analyticsData){{for(const row of (analyticsSummaryData?.daily_usage||[])){{for(const person of (row.people||[]))pushUse(person,row.day,row.day,"","轻量实时统计")}}}}return out}}
...
function creatorUsageText(dayOrBuckets,bucket=null){{const compact=analyticsSummaryData?.daily_usage||[];if(bucket){{if(!creatorUsageDayIsClean(dayOrBuckets))return "/";const row=compact.find(item=>item.day===dayOrBuckets);return row?String(Number(row.active_users||0)):String(bucket.users.size)}}const days=new Set(dayOrBuckets.map(([day])=>day));const compactRows=compact.filter(item=>days.has(item.day)&&creatorUsageDayIsClean(item.day));if(compactRows.length){{const users=new Set();for(const row of compactRows)for(const person of (row.people||[]))users.add(analyticsEntityKey(person)||person.account_id||person.phone||person.kwai_id);return String([...users].filter(Boolean).length)}}const users=new Set();let hasClean=false;for(const [,b] of dayOrBuckets){{if(!creatorUsageDayIsClean(b.day))continue;hasClean=true;for(const key of b.users.keys())users.add(key)}}return hasClean?String(users.size):"/"}}
...
function creatorOverviewMetrics(){{const rows=creatorDisplayedRows();const registered=accounts.filter(a=>creatorAccountIsRegistered(a)&&!analyticsIsTestUser(a));const registeredKeys=new Set(registered.map(a=>String(a.account_id||a.phone||"")));const metricRows=rows.filter(c=>!creatorIsTestRow(c)&&(c.registration_status==="registered"||registeredKeys.has(String(c.account_id||c.phone||""))||creatorFeedReturnCount(c)>0||creatorFeedCount(c)>0));const live=analyticsSummaryData?.summary||{{}};return {{metricRows,registered:Number.isFinite(Number(live.registered_users))?Number(live.registered_users):registered.length,scriptCount:Number.isFinite(Number(live.script_opens))?Number(live.script_opens):creatorAnalyticsScriptOpenTotal(metricRows),returnCount:Number.isFinite(Number(live.submissions))?Number(live.submissions):metricRows.reduce((sum,c)=>sum+creatorFeedReturnCount(c),0)}}}}
function creatorSummaryCardsHtml(){{const data=creatorOverviewMetrics();const source=analyticsSummaryData?.from_cache?"缓存摘要":"实时查询";const live=analyticsSummaryData?.summary||{{}};const raw=Number(live.raw_script_opens||0);const unique=Number(live.unique_scripts_opened||0);const openNote=raw?`有效账号打开 ${{data.scriptCount}} 次 · 原始事件 ${{raw}} 次 · ${{source}}`:`有效账号打开次数 · ${{source}}`;return `<section id="creator-core-summary" class="analytics-kpis analytics-core-kpis" style="margin:0 0 16px">${{analyticsSummaryCard("注册用户",data.registered,`已排除测试账号 666 · ${{source}}`)}}${{analyticsSummaryCard("脚本打开次数",data.scriptCount,`${{openNote}}${{unique?` · 涉及 ${{unique}} 个脚本`:""}}`)}}${{analyticsSummaryCard("回传数",data.returnCount,`按账号归类的真实回传 · ${{source}}`)}}</section>`}}
function refreshCreatorSummaryCards(){{const node=document.querySelector("#creator-core-summary");if(node)node.outerHTML=creatorSummaryCardsHtml()}}
async function loadCreatorSummary(opts={{}}){{if(analyticsSummaryLoading)return;analyticsSummaryLoading=true;try{{const data=await api(`/api/creator-admin/analytics-summary?days=180&_=${{Date.now()}}`);analyticsSummaryData=data;if(activeTab==="creators"&&!selectedCreatorId&&document.querySelector("#creator-list"))renderCreators();else refreshCreatorSummaryCards();const status=document.querySelector("#status");if(status&&!opts.silent)status.textContent=`三项实时数据已更新：${{new Date().toLocaleTimeString("zh-CN",{{hour12:false}})}}`;}}catch(e){{const status=document.querySelector("#status");if(status&&!opts.silent)status.textContent=`实时摘要暂时不可用：${{e.message||e}}`}}finally{{analyticsSummaryLoading=false}}}}
function creatorOverviewHtml(){{const data=creatorOverviewMetrics();return `${{creatorSummaryCardsHtml()}}${{creatorDailyDetailsHtml(data.metricRows)}}${{creatorPocStatsHtml(data.metricRows)}}`}}
...
function openEdit(id){{editing=entries.find(e=>e.entry_id===id);if(!editing)return;const feedback=document.querySelector("#script-taxonomy-feedback");if(feedback){{feedback.textContent="";feedback.style.color=""}}renderTaxonomyEditor();modal.classList.add("open")}}
async function saveEdit(ev){{ev.preventDefault();if(!editing)return;const feedback=document.querySelector("#script-taxonomy-feedback");const saveButton=document.querySelector("#script-taxonomy-save");const originalText=saveButton?.textContent||"保存标签";const payload={{taxonomy_version:scriptTaxonomy.version,taxonomy_source:"manual"}};for(const dimension of Object.keys(taxonomyDimensionMeta)){{const selected=[...form.querySelectorAll(`[data-taxonomy-edit="${{dimension}}"].active`)].map(button=>button.dataset.taxonomyId);if(dimension==="duration")payload.duration_bucket=selected[0]||"";else payload[`${{dimension}}_tags`]=selected}}try{{if(saveButton){{saveButton.disabled=true;saveButton.textContent="保存中..."}}if(feedback){{feedback.textContent="正在保存并同步标签...";feedback.style.color="#99520f"}}const result=await api(`/api/library/${{editing.entry_id}}/taxonomy`,{{method:"POST",body:JSON.stringify(payload)}});const index=entries.findIndex(entry=>entry.entry_id===editing.entry_id);if(index>=0)entries[index]=result.entry||{{...entries[index],...payload}};renderScriptTaxonomyFilters();renderList();if(feedback){{feedback.textContent="保存成功，新标签已同步到脚本库。";feedback.style.color="#15803d"}}setTimeout(()=>modal.classList.remove("open"),650)}}catch(err){{if(feedback){{feedback.textContent=`保存失败：${{err.message||String(err)}}`;feedback.style.color="#c9481e"}}}}finally{{if(saveButton){{saveButton.disabled=false;saveButton.textContent=originalText}}}}}}
...
async function logout(){{await api("/koko_admin_2026-kwai/logout",{{method:"POST",body:"{}"}}).catch(()=>null);location.reload()}}
...
class AppHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        ...
        if parsed.path == "/library":
            ...
            return
        if parsed.path == "/koko_admin_2026-kwai/scripts":
        ...
        if parsed.path == "/api/creator-admin/analytics":
            ...
            self.send_json(payload, status=status)
            return
        if parsed.path == "/api/creator-admin/analytics-summary":
            if not has_creator_admin_access(self):
                self.send_json({"error": "请先登录 Creator 运营后台后查看实时摘要。"}, status=401)
                return
            query = urllib.parse.parse_qs(parsed.query)
            try:
                days = max(1, min(180, int((query.get("days") or ["180"])[0] or "180")))
            except Exception:
                days = 180
            status, payload = creator_admin_remote_json(
                f"/api/admin/analytics-summary?{urllib.parse.urlencode({'days': days})}",
                timeout=15,
            )
            if status == 200:
                save_creator_admin_cache(CREATOR_ADMIN_ANALYTICS_SUMMARY_CACHE_FILE, payload)
            elif status >= 500:
                cached = creator_admin_cached_payload(
                    CREATOR_ADMIN_ANALYTICS_SUMMARY_CACHE_FILE,
                    payload.get("error") or "Creator analytics summary is temporarily unavailable.",
                )
                if cached:
                    status, payload = cached
            self.send_json(payload, status=status)
        ...
        if parsed.path == "/api/creator-admin/accounts":
            ...
            remote_path = "/api/admin/accounts?compact=1" if compact else "/api/admin/accounts"
            status, payload = creator_admin_remote_json(remote_path, timeout=20 if compact else 45)
            if status == 200:
    ...
    def do_POST(self) -> None:
        ...
        if parsed.path == "/koko_admin_2026-kwai/login":
            ...
            return
        if parsed.path == "/koko_admin_2026-kwai/logout":
        ...
        if parsed.path == "/api/creator-admin/creators":
            ...
            if status < 400:
                refresh_status, refresh_payload = creator_admin_remote_json("/api/admin/creators", timeout=20)
                if refresh_status == 200:
                    save_creator_admin_cache(CREATOR_ADMIN_CREATORS_CACHE_FILE, refresh_payload)
            self.send_json(result, status=status)
            return
        ...
        if item_match:
            ...
            if action == "confirm-library":
                ...
                try:
                    ...
                    apply_manual_item_taxonomy(parent_job_id, item_index, payload)
