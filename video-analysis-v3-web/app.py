...
function legacyLocalJson(key,fallback){{try{{const raw=localStorage.getItem(key);return raw?JSON.parse(raw):fallback}}catch(_e){{return fallback}}}}
async function migrateLegacyCreatorCloudState(){{for(const c of creators){{const id=c.profile_id;if(!id)continue;const current=creatorState(id);const patch={{}};const oldFeeds=legacyLocalJson(`kokoCreatorFeeds:${{id}}`,[]).map(feedNormalize).filter(Boolean);if((!Array.isArray(current.feeds)||!current.feeds.length)&&oldFeeds.length)patch.feeds=oldFeeds;const oldDeleted=legacyLocalJson(`kokoCreatorDeletedFeeds:${{id}}`,[]);if((!Array.isArray(current.deleted_feed_keys)||!current.deleted_feed_keys.length)&&Array.isArray(oldDeleted)&&oldDeleted.length)patch.deleted_feed_keys=oldDeleted;const oldMetrics=legacyLocalJson(`kokoCreatorMetrics:${{id}}`,null);if(!current.metrics&&oldMetrics&&typeof oldMetrics==="object")patch.metrics={{...defaultCreatorMetrics(),...oldMetrics}};if(Object.keys(patch).length){{setCreatorState(id,patch);await saveCreatorCloudState(id,patch).catch(()=>null)}}}}}}
async function loadCreators(){{try{{document.querySelector("#status")&&(document.querySelector("#status").textContent="加载作者中...");const d=await api("/api/creator-admin/creators");creators=d.creators||[];await loadCreatorCloudState();await migrateLegacyCreatorCloudState();activeTab="creators";adminView();document.querySelector("#status").textContent=`共 ${{creators.length}} 个创作者`}}catch(e){{loginView(e.message)}}}}
...
async function saveCreatorMetrics(id){{const input=document.getElementById(`creator-metrics-input-${{id}}`);const status=document.querySelector("#status");try{{const data=JSON.parse(input?.value||"{{}}");const metrics={{...defaultCreatorMetrics(),...data}};setCreatorState(id,{{metrics}});await saveCreatorCloudState(id,{{metrics}});if(status)status.textContent="作者数据已保存到云端。";renderCreators()}}catch(err){{if(status)status.textContent=err instanceof SyntaxError?"作者数据 JSON 格式不正确。":(err.message||"作者数据保存失败。")}}}}
async function saveCreatorFeeds(id){{const input=document.getElementById(`creator-feed-input-${{id}}`);const status=document.querySelector("#status");try{{const oldFeeds=creatorFeeds(id);const next=[...oldFeeds,...parseFeedInput(input?.value||"")];const seen=new Set();const dedup=next.filter(feed=>{{const key=`${{feed.entry_id}}:${{feed.date}}`;if(seen.has(key))return false;seen.add(key);return true}});setCreatorState(id,{{feeds:dedup}});await saveCreatorCloudState(id,{{feeds:dedup}});if(input)input.value="";if(status)status.textContent=`已保存 ${{dedup.length}} 条投喂记录到云端。`;await ensureScriptIndex().catch(()=>null);renderCreators()}}catch(err){{if(status)status.textContent=err.message||"投喂记录保存失败。"}}}}
function updateCreatorFeed(id,entryId,patch){{let found=false;const feeds=creatorFeeds(id).map(feed=>{{if(feed.entry_id!==entryId)return feed;found=true;return {{...feed,...patch}}}});if(!found)feeds.push(feedNormalize({{entry_id:entryId,share_url:scriptShareUrl(entryId),date:feedDate(),...patch}}));setCreatorState(id,{{feeds:feeds.filter(Boolean)}});hydrateCreatorFeeds(id)}}
