...
const labels=["待分类","骗子","偷奸耍滑","整蛊","夫妻吵架","夫妻欺骗","夫妻算计","妻管严","赖账","撬墙角","夫妻出轨","夫妻整蛊","偷吃东西","夫妻关系","整蛊恶搞","骗局反转","赖账/金钱冲突","偷吃/偷懒/耍小聪明","热门"];let entries=[];let creators=[];let submissions=[];let activeTab="scripts";let editing=null;const app=document.querySelector("#app");const modal=document.querySelector("#edit-modal");const form=document.querySelector("#edit-form");
...
async function loadCreators(){{try{{document.querySelector("#status")&&(document.querySelector("#status").textContent="加载作者和回传统计中...");const [d,s]=await Promise.all([api("/api/creator-admin/creators"),api("/api/creator-admin/submissions").catch(()=>({{submissions:[]}}))]);creators=d.creators||[];submissions=s.submissions||[];activeTab="creators";adminView();document.querySelector("#status").textContent=`共 ${{creators.length}} 个创作者，${{submissions.length}} 条回传`}}catch(e){{loginView(e.message)}}}}
...
function groupedSubmissions(){{const map=new Map();for(const s of submissions){{const id=s.entry_id||"unknown";if(!map.has(id))map.set(id,[]);map.get(id).push(s)}}return [...map.entries()].sort((a,b)=>b[1].length-a[1].length)}}
function renderSubmissionStats(){{const box=document.querySelector("#submission-stats");if(!box)return;const groups=groupedSubmissions();if(!groups.length){{box.innerHTML=`<section class="submission-summary"><div><h2>回传收集统计</h2><p class="copy">还没有收到创作者提交的视频外链。用户在脚本详情页粘贴链接后，会自动出现在这里。</p></div><div class="submission-count"><b>0</b><span>条回传</span></div></section>`;return}}box.innerHTML=`<section class="submission-summary"><div><h2>回传收集统计</h2><p class="copy">按具体脚本聚合展示，方便运营查看每个脚本收到了哪些创作者成片。</p></div><div class="submission-count"><b>${{submissions.length}}</b><span>条回传</span></div></section><div class="submission-groups">${{groups.map(([id,items])=>{{const first=items[0]||{{}};const title=first.script_title||entries.find(e=>e.entry_id===id)?.title||id;return `<article class="submission-group"><h3>${{esc(title)}} <span class="pill">${{items.length}} 条</span></h3><div class="small">脚本 ID：${{esc(id)}} · 分类：${{esc(first.script_content_type||"未分类")}}</div>${{items.map(s=>`<div class="submission-row"><div><a href="${{esc(s.video_url)}}" target="_blank" rel="noopener">${{esc(s.submitted_title||s.video_url)}}</a><div class="small">创作者：${{esc(s.creator_id||"未绑定账号")}} · ${{esc(timeText(s.created_at))}} · ${{esc(s.status||"pending_review")}}</div></div><button type="button" data-copy="${{esc(s.video_url)}}">复制外链</button></div>`).join("")}}</article>`}}).join("")}}</div>`}}
function renderCreators(){{renderSubmissionStats();const list=document.querySelector("#creator-list");if(!list)return;list.innerHTML=creators.length?creators.map(creatorCard).join(""):`<div class="empty">还没有创作者。先导入一个 Kwai 作者主页链接。</div>`}}
