...
function analyticsPersonLine(item,kind=""){{const name=item.display_name||item.account_id||item.visitor_id||"匿名访客";const account=[item.phone&&`手机号 ${item.phone}`,item.kwai_id&&`Kwai ${item.kwai_id}`,item.account_id&&`账号 ${item.account_id}`].filter(Boolean).join(" · ");const script=item.script_title||item.script_id||"";const source=[item.source,item.path&&`路径 ${item.path}`,item.referer&&`来源 ${item.referer}`].filter(Boolean).join(" · ");const duration=Number(item.duration_seconds||0);return `<div class="analytics-person"><b>${esc(name)}</b>${account?`<small>${esc(account)}</small>`:""}${script?`<small>脚本：${esc(script)}</small>`:""}${duration?`<small>停留：${secondsText(duration)}</small>`:""}${source?`<small>${esc(source)}</small>`:""}${item.time?`<small>时间：${esc(timeText(item.time))}</small>`:""}</div>`}};
...
function analyticsCoreTimelineHtml(){{const rows=(analyticsData?.timeline?.hourly||[]).slice().sort((a,b)=>String(b.hour||"").localeCompare(String(a.hour||"")));if(!rows.length)return `<details class="analytics-core-detail"><summary>展开小时明细 <small>当前接口还没有返回小时数据</small></summary><div class="empty" style="margin-top:12px">暂无可展示的小时明细。等待 kokocomedy 数据接口更新后会自动显示。</div></details>`;return `<details class="analytics-core-detail"><summary>展开小时明细 <small>${rows.length} 个小时段，可以继续点开看具体账号</small></summary><div class="analytics-hour-table"><div class="analytics-hour-head"><span>时间</span><span>注册</span><span>直接打开</span><span>分享链接打开</span><span>脚本打开</span><span>平均时长</span></div>${rows.map(row=>`<div class="analytics-hour-row"><span data-label="时间">${esc(analyticsHourLabel(row.hour))}</span><b data-label="注册">${Number(row.registered_users||0)}</b><b data-label="直接打开">${Number(row.platform_opens||0)} / ${Number(row.platform_people_count||0)}人</b><b data-label="分享链接打开">${Number(row.share_link_opens||0)} / ${Number(row.script_people_count||0)}人</b><b data-label="脚本打开">${Number(row.script_opens||0)}</b><span data-label="平均时长">${secondsText(row.avg_script_duration_seconds||0)}</span><details><summary>查看这个小时的具体账号、来源和停留</summary><div class="analytics-hour-detail">${analyticsDetailGroup("注册用户",row.registered_details)}${analyticsDetailGroup("直接打开 Koko",row.platform_open_details)}${analyticsDetailGroup("打开脚本",row.script_open_details)}${analyticsDetailGroup("停留行为",row.duration_details)}</div></details></div>`).join("")}</div></details>`}}
...
function renderAnalytics(){const box=document.querySelector("#analytics-board");if(!box)return;if(!analyticsData){box.innerHTML=`<div class="empty">正在自动加载有行为用户。无行为账号会保持折叠，展开时再加载。</div>`;return}const s=analyticsData.summary||{};const users=analyticsData.users||[];const inactive=analyticsData.inactive_users||[];const inactiveTotal=Number(s.inactive_accounts||inactive.length||0);const registeredUsers=users.filter(u=>analyticsIsRegistered(u));const firstSubmissionRegistered=users.filter(u=>u.registration_status!=="registered"&&Number(u.submission_count||0)>0).length;const platformOpens=Number(s.platform_opens||0);const scriptOpens=Number(s.script_opens||0);const kokoOpens=platformOpens+scriptOpens;const scriptDurationSeconds=users.reduce((sum,u)=>sum+Number(u.script_duration_seconds||0),0);const avgScriptOpenSeconds=scriptOpens?scriptDurationSeconds/scriptOpens:0;const kpis=[["注册用户",registeredUsers.length,`已激活/新注册 ${Number(s.registered||0)} · 首次回传计入 ${firstSubmissionRegistered}`],["Koko 打开次数",kokoOpens,`直接打开 kokocomedy ${platformOpens} · 分享链接打开 ${scriptOpens}`],["脚本打开次数",scriptOpens,`平均打开时长 ${secondsText(avgScriptOpenSeconds)} · 有行为用户 ${users.length}`]];box.innerHTML=`<div class="analytics-shell"><section class="analytics-kpis analytics-core-kpis">${kpis.map(([label,value,sub])=>analyticsSummaryCard(label,value,sub)).join("")}</section>${analyticsCoreTimelineHtml()}<section class="analytics-layout"><div class="analytics-panel"><div class="analytics-panel-title"><h3>拉新转化漏斗</h3><small>近 ${analyticsData.days||30} 天</small></div>${analyticsFunnelHtml(s,users)}</div><div class="analytics-panel"><div class="analytics-panel-title"><h3>留存分析</h3><small>只展示有行为用户</small></div>${analyticsRetentionHtml(users)}</div></section><section class="analytics-layout"><div class="analytics-panel"><div class="analytics-panel-title"><h3>作者状态</h3>${analyticsFilterTabs()}</div>${analyticsAuthorRows(users)}</div><div class="analytics-panel"><div class="analytics-panel-title"><h3>脚本表现排行</h3><small>按回传、打开、停留排序</small></div>${analyticsScriptRanking(users)}</div></section>${analyticsInactiveRows(inactive,inactiveTotal,analyticsData.inactive_loaded)}</div>`;wireAnalyticsControls();const status=document.querySelector("#status");if(status)status.textContent=`核心数据已加载：注册用户 ${registeredUsers.length} 个，Koko 打开 ${kokoOpens} 次，脚本打开 ${scriptOpens} 次。无行为账号 ${inactiveTotal} 个已折叠。`}
