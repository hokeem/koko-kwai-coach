...
function creatorFeedPanel(c){{const feeds=creatorFeeds(c.profile_id);const byDate=new Map();for(const feed of feeds)byDate.set(feed.date,(byDate.get(feed.date)||0)+1);const stats=[`<span class="pill">总投喂 ${{feeds.length}}</span>`,...[...byDate.entries()].sort((a,b)=>b[0].localeCompare(a[0])).slice(0,6).map(([date,count])=>`<span class="pill">${{esc(date)}}：${{count}} 条</span>`)].join("");return `<section class="import-panel"><h3 style="margin:0;color:#ff8200">投喂脚本及回收情况统计</h3><div class="feed-stats">${{stats}}</div><details><summary>导入/补充投喂脚本</summary><div class="mini-import"><textarea id="creator-feed-input-${{esc(c.profile_id)}}" placeholder="每行一个：2026-07-22, https://kokocomedy.com/script/..., 未完成, 回传链接（可选）"></textarea><button class="primary" type="button" data-save-creator-feeds="${{esc(c.profile_id)}}">保存投喂</button></div></details><div id="creator-feed-list-${{esc(c.profile_id)}}" class="feed-list">${{feeds.length?feeds.map(feed=>feedScriptCard(feed,c)).join(""):'<div class="empty">还没有投喂记录。先导入 kokocomedy 分享链接。</div>'}}</div></section>`}}
async function hydrateCreatorFeeds(id){{const c=creators.find(x=>x.profile_id===id);const box=document.getElementById(`creator-feed-list-${{id}}`);if(!c||!box)return;try{{await ensureScriptIndex();const feeds=creatorFeeds(id);box.innerHTML=feeds.length?feeds.map(feed=>feedScriptCard(feed,c)).join(""):'<div class="empty">还没有投喂记录。先导入 kokocomedy 分享链接。</div>'}}catch(err){{box.innerHTML=`<div class="empty">${{esc(err.message||err)}}</div>`}}}}
