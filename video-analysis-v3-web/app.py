...
function creatorFeedSummary(c){{const feeds=creatorCombinedFeeds(c);const returned=feeds.filter(feed=>feedReturnDate(feed)).length;return {{feeds,feedCount:feeds.length,returnCount:returned}}}}
