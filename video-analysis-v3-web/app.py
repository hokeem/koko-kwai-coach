...
function creatorDailyDetailsHtml(metricRows){{ensureCreatorDailyStyles();const map=new Map();for(const item of creatorDailyUseItems()){{const bucket=creatorDailyBucket(map,item.day);bucket.uses.push(item);if(item.person_key)bucket.users.set(item.person_key,item)}}for(const item of creatorDailyFeedItems(metricRows))creatorDailyBucket(map,item.day).feeds.push(item);for(const item of creatorDailyReturnItems(metricRows))creatorDailyBucket(map,item.day).returns.push(item);const days=[...map.entries()].filter(([day,b])=>day&&day!=="未记录日期"&&(b.users.size||b.feeds.length||b.returns.length)).sort((a,b)=>String(b[0]).localeCompare(String(a[0])));if(!days.length)return `<details class="creator-daily-detail"><summary><span>查看具体数据</span><small>暂无脚本打开、投喂或回传明细；已排除 666</small></summary><div class="creator-daily-list"><div class="empty">目前还没有可展开的每日数据。</div></div></details>`;return `<details class="creator-daily-detail"><summary><span>查看具体数据</span><small>按日统计使用、投喂和回传，实时同步，已排除 666</small></summary><div class="creator-daily-list">${{days.map(([day,bucket])=>{{const uses=bucket.uses.slice().sort((a,b)=>String(a.time||"").localeCompare(String(b.time||"")));const feeds=bucket.feeds.slice().sort((a,b)=>String(a.feed_time||"").localeCompare(String(b.feed_time||"")));const returns=bucket.returns.slice().sort((a,b)=>String(a.return_time||"").localeCompare(String(b.return_time||"")));return `<details class="creator-day-detail"><summary><b>${{esc(day.replaceAll("-","."))}}</b><span>使用 ${{bucket.users.size}} 人</span><span>投喂 ${{feeds.length}} 个</span><span>回传 ${{returns.length}} 个</span></summary><div class="creator-day-sections">${{creatorDailySection("当日使用 Koko 的人",uses,item=>`<div class="creator-day-item"><b>${{esc(item.person_name)}}</b><small>${{esc(timeText(item.time))}} · ${{esc(item.source||item.path||"打开脚本")}}</small><small>脚本：${{esc(item.script_title)}}</small></div>`,"当天没有脚本打开记录。")}}${{creatorDailySection("当日投喂脚本",feeds,item=>`<div class="creator-day-item"><b>${{esc(item.creator_name)}}</b><small>投喂时间：${{esc(item.feed_time||item.day)}}</small><a href="${{esc(item.share_url)}}" target="_blank" rel="noopener">脚本：${{esc(item.script_title)}}</a></div>`,"当天没有投喂记录。")}}${{creatorDailySection("当日回传脚本",returns,item=>`<div class="creator-day-item"><b>${{esc(item.creator_name)}}</b><small>回传时间：${{esc(item.return_time?timeText(item.return_time):item.day)}}</small><a href="${{esc(item.share_url)}}" target="_blank" rel="noopener">脚本：${{esc(item.script_title)}}</a>${{item.return_url?`<a href="${{esc(item.return_url)}}" target="_blank" rel="noopener">视频：${{esc(item.return_url)}}</a>`:""}}</div>`,"当天没有回传记录。")}}</div></details>`}}).join("")}}</div></details>`}}
