...
function feedNormalize(item){{if(!item||typeof item!=="object")return null;const share=String(item.share_url||item.url||item.script_url||"").trim();const id=(share.match(/\\/script\\/([0-9a-f]{{32}})/)||[])[1]||String(item.entry_id||"").trim();if(!id)return null;return {{entry_id:id,share_url:share||scriptShareUrl(id),date:String(item.date||feedDate()).slice(0,10),status:String(item.status||"未完成"),return_url:String(item.return_url||item.video_url||""),return_time:String(item.return_time||"")}}}}
...
function feedStatsHtml(feeds){{const byDate=new Map();for(const feed of feeds)byDate.set(feed.date,(byDate.get(feed.date)||0)+1);return [`<span class="pill">总投喂 ${{feeds.length}}</span>`,...[...byDate.entries()].sort((a,b)=>b[0].localeCompare(a[0])).slice(0,6).map(([date,count])=>`<span class="pill">${{esc(date)}}：${{count}} 条</span>`)].join("")}}
function feedsFromSubmissions(c){{return (c.submissions||[]).filter(s=>s.status!=="placeholder"&&s.entry_id).map(s=>feedNormalize({{entry_id:s.entry_id,share_url:scriptShareUrl(s.entry_id),date:String(s.created_at||feedDate()).slice(0,10),status:"已回传",return_url:s.video_url||"",return_time:s.created_at||""}})).filter(Boolean)}}
function feedScriptCard(feed,c){{const entry=scriptIndex[feed.entry_id]||{{}};const ret=feedReturn(c,feed.entry_id);const image=entry.cover_url||entry.thumbnail_url||ret?.thumbnail_url||"";const share=feed.share_url||scriptShareUrl(feed.entry_id);const returnUrl=feed.return_url||ret?.video_url||"";const returnTime=feed.return_time||ret?.created_at||"";return `<article class="feed-card" data-feed-entry="${{esc(feed.entry_id)}}"><img src="${{esc(image)}}" loading="lazy" alt=""><div><h4>${{esc(entry.title||ret?.script_title||feed.entry_id)}}</h4><p>${{esc(entry.summary||"还没有加载到脚本概述，可先保留分享链接。")}}</p><div class="share-line"><b>分享链接</b><a href="${{esc(share)}}" target="_blank" rel="noopener">${{esc(share)}}</a></div></div><div class="feed-controls"><input data-feed-date="${{esc(feed.entry_id)}}" value="${{esc(feed.date)}}"><select data-feed-status="${{esc(feed.entry_id)}}"><option ${{feed.status==="未完成"?"selected":""}}>未完成</option><option ${{feed.status==="正在制作"?"selected":""}}>正在制作</option><option ${{feed.status==="作者不愿意拍摄"?"selected":""}}>作者不愿意拍摄</option><option ${{feed.status==="已回传"?"selected":""}}>已回传</option></select><input data-feed-return-url="${{esc(feed.entry_id)}}" value="${{esc(returnUrl)}}" placeholder="作者回传视频链接"><div class="return-preview"><img src="${{esc(ret?.thumbnail_url||image)}}" alt=""><div><b>${{returnUrl?"已回传":"未回传"}}</b><div class="small">${{esc(returnTime?timeText(returnTime):"等待回传")}}</div>${{returnUrl?`<a class="small" href="${{esc(returnUrl)}}" target="_blank" rel="noopener">打开视频</a>`:""}}</div></div></div></article>`}}
function creatorFeedPanel(c){{const feeds=[...creatorFeeds(c.profile_id),...feedsFromSubmissions(c)].filter((feed,idx,arr)=>arr.findIndex(x=>x.entry_id===feed.entry_id)===idx);return `<section class="import-panel"><h3 style="margin:0;color:#ff8200">投喂脚本及回收情况统计</h3><div id="creator-feed-stats-${{esc(c.profile_id)}}" class="feed-stats">${{feedStatsHtml(feeds)}}</div><details><summary>导入/补充投喂脚本</summary><div class="mini-import"><textarea id="creator-feed-input-${{esc(c.profile_id)}}" placeholder="每行一个：2026-07-22, https://kokocomedy.com/script/..., 未完成, 回传链接（可选）"></textarea><button class="primary" type="button" data-save-creator-feeds="${{esc(c.profile_id)}}">保存投喂</button></div></details><div id="creator-feed-list-${{esc(c.profile_id)}}" class="feed-list">${{feeds.length?feeds.map(feed=>feedScriptCard(feed,c)).join(""):'<div class="empty">正在整理已有投喂和推荐脚本...</div>'}}</div></section>`}}
async function hydrateCreatorFeeds(id){{const c=creators.find(x=>x.profile_id===id);const box=document.getElementById(`creator-feed-list-${{id}}`);const stats=document.getElementById(`creator-feed-stats-${{id}}`);if(!c||!box)return;try{{await ensureScriptIndex();let feeds=[...creatorFeeds(id),...feedsFromSubmissions(c)].filter((feed,idx,arr)=>arr.findIndex(x=>x.entry_id===feed.entry_id)===idx);if(!feeds.length){{const d=await api(`/api/creator-admin/creators/${{id}}/recommendations?limit=5&offset=0`);for(const script of (d.scripts||[])){{if(script?.entry_id)scriptIndex[script.entry_id]=script;feeds.push(feedNormalize({{entry_id:script.entry_id,share_url:`__CREATOR_BASE__${{script.share_url||(`/script/${{script.entry_id}}`)}}`,date:feedDate(),status:"未完成"}}))}}feeds=feeds.filter(Boolean)}}if(stats)stats.innerHTML=feedStatsHtml(feeds);box.innerHTML=feeds.length?feeds.map(feed=>feedScriptCard(feed,c)).join(""):'<div class="empty">还没有投喂记录，也没有匹配到可推荐脚本。</div>'}}catch(err){{box.innerHTML=`<div class="empty">${{esc(err.message||err)}}</div>`}}}}
...
function updateCreatorFeed(id,entryId,patch){{const feeds=creatorFeeds(id).map(feed=>feed.entry_id===entryId?{{...feed,...patch}}:feed);writeLocalJson(creatorStorageKey("kokoCreatorFeeds",id),feeds);hydrateCreatorFeeds(id)}}
