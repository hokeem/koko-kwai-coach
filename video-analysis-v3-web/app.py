...
function analyticsStatusPill(user){{const registered=user.registration_status==="registered";return `<span class="pill ${{registered?"":"off"}}">${{registered?"已注册":"未注册"}}</span>`}}
...
function analyticsRetentionHtml(users){{const rows=[["已注册",users.filter(u=>u.registration_status==="registered")],["未注册有行为",users.filter(u=>u.registration_status!=="registered")],["有回传",users.filter(u=>Number(u.submission_count||0)>0)],["仅浏览",users.filter(u=>Number(u.submission_count||0)===0&&(Number(u.script_share_open_count||0)>0||(u.script_views||[]).length))]];return `<table class="analytics-heat"><thead><tr><th>用户分层</th><th>人数</th><th>脚本打开</th><th>平均停留</th><th>回传</th></tr></thead><tbody>${{rows.map(([label,list])=>{{const opens=list.reduce((a,u)=>a+Number(u.script_share_open_count||0),0);const duration=list.reduce((a,u)=>a+Number(u.script_duration_seconds||0)+Number(u.platform_duration_seconds||0),0);const subs=list.reduce((a,u)=>a+Number(u.submission_count||0),0);return `<tr><td>${{esc(label)}}</td><td><span class="heat h${{Math.min(5,Math.ceil(list.length/5))}}">${{list.length}}</span></td><td><span class="heat h${{Math.min(5,Math.ceil(opens/5))}}">${{opens}}</span></td><td>${{secondsText(analyticsAvgSeconds(duration,list.length))}}</td><td><span class="heat h${{Math.min(5,Math.ceil(subs/8))}}">${{subs}}</span></td></tr>`}}).join("")}}</tbody></table>`}}
function analyticsFilteredUsers(users){{if(analyticsUserFilter==="registered")return users.filter(u=>u.registration_status==="registered");if(analyticsUserFilter==="submitted")return users.filter(u=>Number(u.submission_count||0)>0);if(analyticsUserFilter==="followup")return users.filter(u=>u.registration_status!=="registered"&&(Number(u.script_share_open_count||0)>0||Number(u.submission_count||0)>0));return users}}
function analyticsFilterTabs(){{const tabs=[["all","全部"],["registered","已激活"],["submitted","有回传"],["followup","需跟进"]];return `<div class="analytics-tabs">${{tabs.map(([key,label])=>`<button class="${{analyticsUserFilter===key?"active":""}}" type="button" data-analytics-filter="${{key}}">${{label}}</button>`).join("")}}</div>`}}
function analyticsAuthorRows(users){{const rows=analyticsFilteredUsers(users).slice().sort((a,b)=>(Number(b.submission_count||0)-Number(a.submission_count||0))||(Number(b.script_share_open_count||0)-Number(a.script_share_open_count||0))||(Number(b.platform_open_count||0)-Number(a.platform_open_count||0))).slice(0,12);if(!rows.length)return `<div class="empty">当前筛选下没有作者。</div>`;return `<div class="analytics-table"><div class="analytics-table-head"><span>作者</span><span>状态</span><span>最近访问</span><span>脚本打开</span><span>回传</span><span>操作</span></div>${{rows.map(u=>`<div class="analytics-table-row"><div class="creator-row-main"><div class="avatar"></div><div class="creator-row-title"><b>${{esc(u.display_name||u.account_id||u.phone)}}</b><span class="small">${{esc(u.phone||u.account_id)}} · ${{esc(u.kwai_id||"")}}</span></div></div><div>${{analyticsStatusPill(u)}}</div><div class="small">${{esc(timeText(u.last_event_at||u.last_login_at||u.registered_at||u.created_at)||"-")}}</div><b>${{Number(u.script_share_open_count||0)}}</b><b>${{Number(u.submission_count||0)}}</b><button type="button" data-copy="${{esc(u.phone||u.account_id||"")}}">复制账号</button></div>`).join("")}}</div>`}}
...
function renderAnalytics(){const box=document.querySelector("#analytics-board");if(!box)return;if(!analyticsData){box.innerHTML=`<div class="empty">正在自动加载有行为用户。无行为账号会保持折叠，展开时再加载。</div>`;return}const s=analyticsData.summary||{};const users=analyticsData.users||[];const inactive=analyticsData.inactive_users||[];const inactiveTotal=Number(s.inactive_accounts||inactive.length||0);const submittedUsers=users.filter(u=>Number(u.submission_count||0)>0).length;const kpis=[["预置账号",s.accounts||0,"全部导入账号"],["已激活",s.registered||0,`${analyticsPct(s.registered,s.accounts)}% 激活`],["近30日有行为",s.active_accounts||users.length,`无行为 ${inactiveTotal}`],["平台打开",s.platform_opens||0,"进入 kokocomedy"],["脚本页打开",s.script_opens||0,"外部分享链接"],["完成回传",s.submissions||0,`${submittedUsers} 个作者`]];box.innerHTML=`<div class="analytics-shell"><section class="analytics-kpis">${kpis.map(([label,value,sub])=>analyticsSummaryCard(label,value,sub)).join("")}</section><section class="analytics-layout"><div class="analytics-panel"><div class="analytics-panel-title"><h3>拉新转化漏斗</h3><small>近 ${analyticsData.days||30} 天</small></div>${analyticsFunnelHtml(s,users)}</div><div class="analytics-panel"><div class="analytics-panel-title"><h3>留存分析</h3><small>只展示有行为用户</small></div>${analyticsRetentionHtml(users)}</div></section><section class="analytics-layout"><div class="analytics-panel"><div class="analytics-panel-title"><h3>作者状态</h3>${analyticsFilterTabs()}</div>${analyticsAuthorRows(users)}</div><div class="analytics-panel"><div class="analytics-panel-title"><h3>脚本表现排行</h3><small>按回传、打开、停留排序</small></div>${analyticsScriptRanking(users)}</div></section>${analyticsInactiveRows(inactive,inactiveTotal,analyticsData.inactive_loaded)}</div>`;wireAnalyticsControls();const status=document.querySelector("#status");if(status)status.textContent=`最近 ${analyticsData.days||30} 天：默认显示 ${users.length} 个有行为账号，${inactiveTotal} 个无行为账号已折叠，${s.events||0} 条行为事件。`}
async function loadAnalytics(includeInactive=false){{activeTab="analytics";if(!document.querySelector("#analytics-board"))adminView();const status=document.querySelector("#status");const box=document.querySelector("#analytics-board");if(analyticsLoading)return;analyticsLoading=true;if(status)status.textContent=includeInactive?"正在加载无行为账号...":"正在加载有行为用户...";if(box&&!analyticsData)box.innerHTML=`<div class="empty">正在拉取有行为用户，不会等待无行为账号。</div>`;try{{const data=await api(`/api/creator-admin/analytics?days=30&include_inactive=${{includeInactive?"1":"0"}}`);if(includeInactive&&analyticsData){{analyticsData={{...analyticsData,summary:data.summary||analyticsData.summary,inactive_users:data.inactive_users||[],inactive_loaded:true}};analyticsInactiveLoaded=true}}else{{analyticsData=data;analyticsInactiveLoaded=Boolean(data.inactive_loaded)}}analyticsAutoLoaded=true;renderAnalytics()}}catch(e){{const msg=e.message||"Creator 数据接口暂时不可用";if(status)status.textContent=`数据加载失败：${{msg}}`;if(box&&!analyticsData)box.innerHTML=`<div class="empty">数据加载失败：${{esc(msg)}}<br><br><button class="primary" id="retry-analytics" type="button">重新加载有行为用户</button></div>`;document.querySelector("#retry-analytics")?.addEventListener("click",()=>loadAnalytics(false))}}finally{{analyticsLoading=false}}}}
async function loadCreators(){{try{{document.querySelector("#status")&&(document.querySelector("#status").textContent="加载作者中...");const d=await api("/api/creator-admin/creators");creators=d.creators||[];await loadCreatorCloudState();activeTab="creators";adminView();document.querySelector("#status").textContent=`共 ${{creators.length}} 个创作者`}}catch(e){{loginView(e.message)}}}}
...
function creatorPocCounts(){{const counts=new Map();for(const c of creators){{const poc=creatorPocValue(c)||"未分配";counts.set(poc,(counts.get(poc)||0)+1)}}return counts}}
function renderCreatorPocFilters(){{const box=document.querySelector("#creator-poc-filters");if(!box)return;const counts=creatorPocCounts();const dynamic=[...counts.keys()].filter(x=>x&&x!=="未分配"&&!creatorPocOptions.includes(x)).sort();const options=["",...creatorPocOptions,...dynamic,"未分配"];box.innerHTML=options.map(poc=>`<button class="quick-filter-chip ${{activeCreatorPoc===poc?"active":""}}" type="button" data-poc-filter="${{esc(poc)}}"><span>${{esc(poc||"全部 POC")}}</span> <small>${{poc?Number(counts.get(poc)||0):creators.length}}</small></button>`).join("")}}
function creatorSearchText(c){{return [c.name,c.kwai_id,c.kwai_url,c.uid,c.phone,c.account_id,c.poc,c.owner,c.cooperation_level,c.creator_description,JSON.stringify(c.creator_type||{{}}),JSON.stringify(c.categories||[])].join(" ").toLowerCase()}}
...
function creatorSortedRows(rows){{return [...rows].sort((a,b)=>{{const sa=creatorFeedSummary(a);const sb=creatorFeedSummary(b);return (sb.feedCount-sa.feedCount)||(sb.returnCount-sa.returnCount)||String(a.name||a.kwai_id||"").localeCompare(String(b.name||b.kwai_id||""),"zh-CN")}})}}
...
function filteredCreators(){{const q=String(document.querySelector("#creator-search")?.value||"").trim().toLowerCase();const scope=String(document.querySelector("#creator-search-scope")?.value||"all");let rows=q?creators.filter(c=>String(creatorFieldValue(c,scope)).toLowerCase().includes(q)):creators;if(activeCreatorPoc)rows=rows.filter(c=>(creatorPocValue(c)||"未分配")===activeCreatorPoc);return creatorSortedRows(rows)}}
...
function feedIsReturned(feed){{return String(feed.status||"").trim()==="已回传"}}
...
function creatorTile(c){{const account=c.account||{{}};const avatar=esc(c.avatar_url||creatorFallbackAvatar());const name=c.name||c.kwai_id||"Kwai creator";const kwai=c.kwai_id||"-";const uid=c.uid||account.uid||"-";const phone=c.phone||account.phone||"-";const poc=creatorPocValue(c)||"未分配";const summary=creatorFeedSummary(c);const feedCount=summary.feedCount;const returnCount=summary.returnCount;return `<article class="creator-tile" data-open-creator="${{esc(c.profile_id)}}"><div class="creator-tile-head"><img class="avatar" src="${{avatar}}" alt=""><div><h3 class="creator-tile-name">${{esc(name)}}</h3><div class="small">Kwai ID&nbsp; ${{esc(kwai)}}</div></div></div><div class="meta"><span class="creator-badge">POC ${{esc(poc)}}</span><span class="pill">${{esc(c.cooperation_level||"Normal")}}</span>${{(c.categories||[]).slice(0,2).map(x=>`<span class="pill">${{esc(x)}}</span>`).join("")}}</div><div class="creator-field-lines"><div class="creator-field-line"><span>UID</span><span>${{esc(uid)}}</span></div><div class="creator-field-line"><span>电话</span><span>${{esc(phone)}}</span></div></div><div class="creator-stats-row"><div class="creator-stat"><span>投喂脚本数</span><b>${{feedCount}}</b></div><div class="creator-stat"><span>回传视频数</span><b>${{returnCount}}</b></div></div></article>`}}
function creatorRow(c){{const type=c.creator_type||{{}};const avatar=esc(c.avatar_url||creatorFallbackAvatar());const name=c.name||c.kwai_id||"Kwai creator";const kwai=c.kwai_id||"-";const poc=creatorPocValue(c)||"未分配";const summary=creatorFeedSummary(c);const tags=[`POC ${{poc}}`,c.cooperation_level||"Normal",tagLabel(type.identity),tagLabel(type.location),...(c.categories||[])].filter(Boolean);return `<article class="creator-row" data-open-creator="${{esc(c.profile_id)}}"><div class="creator-row-main"><img class="avatar" src="${{avatar}}" alt=""><div class="creator-row-title"><b>${{esc(name)}}</b><span class="small">@${{esc(kwai)}}</span></div></div><div class="creator-row-num" data-label="投喂脚本数">${{summary.feedCount}}</div><div class="creator-row-num" data-label="回传脚本数">${{summary.returnCount}}</div><div class="creator-row-tags">${{tags.slice(0,8).map(x=>`<span class="pill">${{esc(x)}}</span>`).join("")}}</div></article>`}}
...
function creatorMetricsPanel(c){{const data=creatorMetrics(c.profile_id);return `<section class="import-panel"><div class="feed-stats-head"><h3 style="margin:0;color:#ff8200">作者数据</h3><button type="button" data-save-creator-metrics="${{esc(c.profile_id)}}">保存并刷新</button></div><div class="metrics-grid">${{metricCard("本周打开次数",data.opens)}}${{metricCard("本周发文数量",data.posts)}}${{metricCard("本周总播放量",data.views)}}${{metricCard("本周篇均播放量",data.avgViews)}}</div><details><summary>导入数据</summary><div class="mini-import"><textarea id="creator-metrics-input-${{esc(c.profile_id)}}" placeholder='支持 JSON，例如 {{"opens":{{"value":12,"delta":20}},"posts":{{"value":3,"delta":-25}},"views":{{"value":50000,"delta":18}},"avgViews":{{"value":16666,"delta":8}}}}'>${{esc(JSON.stringify(data,null,2))}}</textarea><button class="primary" type="button" data-save-creator-metrics="${{esc(c.profile_id)}}">保存并刷新</button></div></details></section>`}}
...
async function hydrateCreatorFeeds(id,persist=false){{const c=creators.find(x=>x.profile_id===id);const box=document.getElementById(`creator-feed-list-${{id}}`);const stats=document.getElementById(`creator-feed-stats-${{id}}`);if(!c||!box)return;try{{await ensureScriptIndex();let feeds=[...creatorFeeds(id),...feedsFromSubmissions(c)].filter((feed,idx,arr)=>arr.findIndex(x=>x.entry_id===feed.entry_id)===idx);if(!feeds.length){{const d=await api(`/api/creator-admin/creators/${{id}}/recommendations?limit=5&offset=0`);for(const script of (d.scripts||[])){{if(script?.entry_id)scriptIndex[script.entry_id]=script;feeds.push(feedNormalize({{entry_id:script.entry_id,share_url:`__CREATOR_BASE__${{script.share_url||(`/script/${{script.entry_id}}`)}}`,date:feedDate(),status:"未完成"}}))}}feeds=feeds.filter(Boolean)}}feeds=visibleCreatorFeeds(id,feeds);if(persist){{setCreatorState(id,{{feeds}});await saveCreatorCloudState(id,{{feeds}})}}if(stats)stats.innerHTML=feedStatsHtml(feeds,id);box.innerHTML=feeds.length?feeds.map(feed=>feedScriptCard(feed,c)).join(""):'<div class="empty">还没有投喂记录，也没有匹配到可推荐脚本。</div>'}}catch(err){{box.innerHTML=`<div class="empty">${{esc(err.message||err)}}</div>`}}}}
function creatorDetail(c){{const account=c.account||{{}};const type=c.creator_type||{{}};const avatar=esc(c.avatar_url||creatorFallbackAvatar());const name=c.name||c.kwai_id||"Kwai creator";const summary=creatorFeedSummary(c);return `<div class="creator-detail-top"><button type="button" data-back-creators>返回列表</button><div class="actions"><button class="primary" type="button" data-edit-creator-tags="${{esc(c.profile_id)}}">编辑标签</button><button class="danger" type="button" data-delete-creator="${{esc(c.profile_id)}}">删除</button></div></div><section class="creator-detail-hero"><img class="avatar" src="${{avatar}}" alt=""><div><h2 style="margin:0 0 6px">${{esc(name)}}</h2><div class="small">@${{esc(c.kwai_id||"")}} · UID ${{esc(c.uid||account.uid||"未填")}} · 电话 ${{esc(c.phone||account.phone||"未填")}}</div><a class="small" href="${{esc(c.kwai_url)}}" target="_blank" rel="noopener">${{esc(c.kwai_url||"")}}</a><div class="meta"><span class="pill">POC ${{esc(creatorPocValue(c)||"未分配")}}</span><span class="pill">配合度 ${{esc(c.cooperation_level||"待标注")}}</span><span class="pill">身份 ${{esc(tagLabel(type.identity))}}</span><span class="pill">地点 ${{esc(tagLabel(type.location))}}</span>${{(c.categories||[]).map(x=>`<span class="pill">${{esc(x)}}</span>`).join("")}}</div></div><div class="submission-count"><b>${{summary.returnCount}}</b><span>回传</span></div></section><div class="creator-detail-stack"><section class="import-panel creator-recommend-panel"><h3 style="margin:0;color:#ff8200">推荐脚本</h3><p class="small">按内容标签、身份标签和地点标签现场计算。一次展示 5 个，点击脚本卡片打开 kokocomedy 分享页。</p><button type="button" data-load-creator-scripts="${{esc(c.profile_id)}}">查看推荐脚本</button><div id="creator-scripts-${{esc(c.profile_id)}}" class="grid" data-offset="0"></div></section>${{creatorMetricsPanel(c)}}${{creatorFeedPanel(c)}}</div>`}}
function renderCreators(){{const list=document.querySelector("#creator-list");if(!list)return;updateCreatorViewToggle();const rows=filteredCreators();if(selectedCreatorId&&!creators.some(c=>c.profile_id===selectedCreatorId))selectedCreatorId="";const detail=selectedCreatorId?creators.find(c=>c.profile_id===selectedCreatorId):null;if(detail){{list.innerHTML=creatorDetail(detail);hydrateCreatorFeeds(detail.profile_id);const meta=document.querySelector("#creator-results-meta");if(meta)meta.innerHTML=`Total of <b>${{creators.length}}</b> · 当前作者：${{esc(detail.name||detail.kwai_id||detail.profile_id)}}`;const s=document.querySelector("#status");if(s)s.textContent="点击“查看推荐脚本”后才会加载推荐，避免列表卡顿。";return}}const total=rows.length;const pages=Math.max(1,Math.ceil(total/CREATOR_PAGE_SIZE));creatorPage=Math.min(Math.max(1,creatorPage),pages);const start=(creatorPage-1)*CREATOR_PAGE_SIZE;const pageRows=rows.slice(start,start+CREATOR_PAGE_SIZE);const meta=document.querySelector("#creator-results-meta");if(meta)meta.innerHTML=`Total of <b>${{total}}</b> | 按投喂脚本数降序 | kwaiId(${{rows.filter(c=>c.kwai_id).length}}) | 用户名(${{rows.filter(c=>c.name).length}})`;if(!pageRows.length){{list.innerHTML=`<div class="empty">没有匹配创作者。点击“导入创作者”添加一个 Kwai 作者主页。</div>`;return}}const body=creatorViewMode==="list"?`<div class="creator-row-list"><div class="creator-row-head"><span>作者</span><span>投喂脚本数</span><span>回传脚本数</span><span>标签</span></div>${{pageRows.map(creatorRow).join("")}}</div>`:`<div class="creator-card-grid">${{pageRows.map(creatorTile).join("")}}</div>`;list.innerHTML=`${{body}}<div class="creator-pagination"><button type="button" data-creator-page="prev" ${{creatorPage<=1?"disabled":""}}>‹</button><span class="pill">${{creatorPage}} / ${{pages}}</span><button type="button" data-creator-page="next" ${{creatorPage>=pages?"disabled":""}}>›</button><span class="small">18/page</span></div>`;const s=document.querySelector("#status");if(s)s.textContent=""}}
...
async function saveCreatorTags(formEl){{const id=formEl?.dataset?.creatorTags||"";if(!id)return;const fd=new FormData(formEl);const status=document.querySelector("#status");if(status)status.textContent="正在保存作者标签...";const identities=selectedValues(formEl,"identity");const locations=selectedValues(formEl,"location");const payload={{kwai_url:fd.get("kwai_url"),display_name:fd.get("display_name"),kwai_id:fd.get("kwai_id"),phone:fd.get("phone"),uid:fd.get("uid"),poc:fd.get("poc"),categories:fd.get("category")?[fd.get("category")]:[],creator_type:{{identity:identities,location:locations}},identity:identities,location:locations,cooperation_level:fd.get("cooperation_level"),creator_description:fd.get("creator_description"),skip_fetch:true}};try{{const d=await api(`/api/creator-admin/creators/${{id}}`,{{method:"POST",body:JSON.stringify(payload)}});const updated=d.creator;if(updated){{const index=creators.findIndex(x=>x.profile_id===id);if(index>=0)creators[index]=updated}}creatorTagsModal.classList.remove("open");renderCreatorPocFilters();renderCreators();if(status)status.textContent="作者标签已保存。点击“查看推荐脚本”会按新标签重新计算推荐。"}}catch(err){{if(status)status.textContent=err.message}}}}
async function loadCreatorScripts(id,advance=false){{const box=document.getElementById(`creator-scripts-${{id}}`);if(!box)return;const current=Number(box.dataset.offset||0);const offset=advance?current:0;box.dataset.offset=String(offset);box.innerHTML=`<div class="empty">正在按标签计算推荐脚本...</div>`;try{{const d=await api(`/api/creator-admin/creators/${{id}}/recommendations?limit=5&offset=${{offset}}`);const scripts=d.scripts||[];box.innerHTML=scripts.length?`<div class="creator-script-grid">${{scripts.slice(0,5).map(scriptMini).join("")}}</div><div class="recommend-footer"><button class="primary" type="button" data-load-more-creator-scripts="${{esc(id)}}">更新推荐</button></div>`:`<div class="empty">暂时没有匹配脚本。请先调整作者分类标签后重试。</div>`;box.dataset.offset=String(offset+scripts.length);const status=document.querySelector("#status");if(status)status.textContent=`已为该作者加载第 ${{offset+1}}-${{offset+scripts.length}} 个推荐脚本，共 ${{Number(d.total||scripts.length||0)}} 个候选。`;}}catch(err){{box.innerHTML=`<div class="empty">${{esc(err.message||err)}}</div>`}}}}
