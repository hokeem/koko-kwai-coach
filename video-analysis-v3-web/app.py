...
function renderAnalytics(){{const box=document.querySelector("#analytics-board");if(!box)return;if(!analyticsData){{box.innerHTML=`<div class="empty">页面已经打开。点击“刷新数据”后再拉取 kokocomedy 使用数据，避免数据接口慢时卡住整个后台。</div>`;return}}const s=analyticsData.summary||{{}};const users=analyticsData.users||[];box.innerHTML=`<section class="analytics-summary">${{analyticsSummaryCard("账号数",s.accounts||0)}}${{analyticsSummaryCard("已注册",s.registered||0)}}${{analyticsSummaryCard("未注册",s.unregistered||0)}}${{analyticsSummaryCard("平台打开",s.platform_opens||0)}}${{analyticsSummaryCard("分享页打开",s.script_opens||0)}}${{analyticsSummaryCard("回传",s.submissions||0)}}</section><section class="creator-row-list"><div class="analytics-row-head"><span>用户</span><span>状态</span><span>平台打开</span><span>分享页打开</span><span>平台停留</span><span>脚本停留</span><span>点击/回传</span></div>${{users.map(user=>`<article class="analytics-row"><div class="creator-row-main"><div class="avatar"></div><div class="creator-row-title"><b>${{esc(user.display_name||user.account_id||user.phone)}}</b><span class="small">${{esc(user.phone||user.account_id)}} · 注册时间：${{esc(timeText(user.registered_at||user.provisioned_at||user.created_at))}}</span></div></div><div>${{analyticsStatusPill(user)}}</div><div class="creator-row-num" data-label="平台打开">${{Number(user.platform_open_count||0)}}</div><div class="creator-row-num" data-label="分享页打开">${{Number(user.script_share_open_count||0)}}</div><div>${{secondsText(user.platform_duration_seconds||0)}}</div><div>${{secondsText(user.script_duration_seconds||0)}}</div><div class="analytics-mini">${{analyticsClickPills(user)}}<span class="pill">回传 ${{Number(user.submission_count||0)}}</span></div><details><summary>查看脚本浏览、点击和回传明细</summary><div class="creator-detail-grid"><section><h3>脚本浏览</h3>${{analyticsScriptRows(user)}}</section><section><h3>回传记录</h3>${{analyticsSubmissionRows(user)}}</section></div><div class="small" style="margin-top:10px">最近行为：${{(user.recent_events||[]).slice(0,8).map(e=>`${{esc(timeText(e.created_at))}} / ${{esc(e.event)}} / ${{esc(e.path||"")}}`).join("<br>")||"暂无"}}</div></details></article>`).join("")}}</section>`;const status=document.querySelector("#status");if(status)status.textContent=`最近 ${{analyticsData.days||30}} 天：${{users.length}} 个账号，${{s.events||0}} 条行为事件。`}}
