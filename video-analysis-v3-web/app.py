...
async function loadCurrentTab(){{if(activeTab==="creators")return loadCreators();if(activeTab==="accounts")return loadAccounts();if(activeTab==="submissions")return loadSubmissions();if(activeTab==="intakes")return loadIntakes();if(activeTab==="imports"){{adminView();return}}return loadEntries()}}
...
function creatorStorageKey(prefix,id){{return `${{prefix}}:${{id}}`}}
function readLocalJson(key,fallback){{try{{const raw=localStorage.getItem(key);return raw?JSON.parse(raw):fallback}}catch(_e){{return fallback}}}}
...
function creatorMetrics(id){{return readLocalJson(creatorStorageKey("kokoCreatorMetrics",id),defaultCreatorMetrics())}}
...
function creatorFeeds(id){{return readLocalJson(creatorStorageKey("kokoCreatorFeeds",id),[]).map(feedNormalize).filter(Boolean)}}
...
function deletedFeedKeys(id){{return new Set(readLocalJson(creatorStorageKey("kokoCreatorDeletedFeeds",id),[]))}}
...
function renderAccounts(){{const list=document.querySelector("#account-list");if(!list)return;const rows=filteredAccounts();list.innerHTML=rows.length?rows.map(accountCard).join(""):`<div class="empty">还没有匹配账号。可以在上方输入数字或字母创建。</div>`}}
...
function saveCreatorMetrics(id){{const input=document.getElementById(`creator-metrics-input-${{id}}`);const status=document.querySelector("#status");try{{const data=JSON.parse(input?.value||"{{}}");writeLocalJson(creatorStorageKey("kokoCreatorMetrics",id),{{...defaultCreatorMetrics(),...data}});if(status)status.textContent="作者数据已保存到本地。";renderCreators()}}catch(err){{if(status)status.textContent="作者数据 JSON 格式不正确。"}}}}
async function saveCreatorFeeds(id){{const input=document.getElementById(`creator-feed-input-${{id}}`);const status=document.querySelector("#status");try{{const oldFeeds=creatorFeeds(id);const next=[...oldFeeds,...parseFeedInput(input?.value||"")];const seen=new Set();const dedup=next.filter(feed=>{{const key=`${{feed.entry_id}}:${{feed.date}}`;if(seen.has(key))return false;seen.add(key);return true}});writeLocalJson(creatorStorageKey("kokoCreatorFeeds",id),dedup);if(input)input.value="";if(status)status.textContent=`已保存 ${{dedup.length}} 条投喂记录。`;await ensureScriptIndex().catch(()=>null);renderCreators()}}catch(err){{if(status)status.textContent=err.message||"投喂记录保存失败。"}}}}
function updateCreatorFeed(id,entryId,patch){{let found=false;const feeds=creatorFeeds(id).map(feed=>{{if(feed.entry_id!==entryId)return feed;found=true;return {{...feed,...patch}}}});if(!found)feeds.push(feedNormalize({{entry_id:entryId,share_url:scriptShareUrl(entryId),date:feedDate(),...patch}}));writeLocalJson(creatorStorageKey("kokoCreatorFeeds",id),feeds.filter(Boolean));hydrateCreatorFeeds(id)}}
function persistCreatorFeedDom(id){{const list=document.getElementById(`creator-feed-list-${{id}}`);if(!list)return creatorFeeds(id);const existing=creatorFeeds(id);const byKey=new Map(existing.map(feed=>[feedKey(feed),feed]));const rows=[];list.querySelectorAll("[data-feed-entry]").forEach(card=>{{const entryId=card.dataset.feedEntry||"";if(!entryId)return;const share=card.querySelector(".share-line a")?.getAttribute("href")||scriptShareUrl(entryId);const date=card.querySelector("[data-feed-date]")?.value||feedDate();const status=card.querySelector("[data-feed-status]")?.value||"未完成";const returnUrl=card.querySelector("[data-feed-return-url]")?.value||"";const returnTime=card.querySelector("[data-feed-return-time]")?.value||"";const prior=byKey.get(card.dataset.feedKey||"")||{{}};rows.push(feedNormalize({{...prior,entry_id:entryId,share_url:share,date,feed_time:date,status,return_url:returnUrl,return_time:returnTime}}))}});const merged=rows.filter(Boolean);writeLocalJson(creatorStorageKey("kokoCreatorFeeds",id),merged);return merged}}
function deleteCreatorFeed(id,key){{const deleted=[...deletedFeedKeys(id),key];writeLocalJson(creatorStorageKey("kokoCreatorDeletedFeeds",id),deleted);const feeds=creatorFeeds(id).filter(feed=>feedKey(feed)!==key);writeLocalJson(creatorStorageKey("kokoCreatorFeeds",id),feeds);hydrateCreatorFeeds(id);const status=document.querySelector("#status");if(status)status.textContent="已删除这条投喂/回传记录。"}}
