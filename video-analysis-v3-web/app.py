...
function creatorMetricsPanel(c){{const data=creatorMetrics(c.profile_id);return `<section class="import-panel"><h3 style="margin:0;color:#ff8200">作者数据</h3><div class="metrics-grid">${{metricCard("本周打开次数",data.opens)}}${{metricCard("本周发文数量",data.posts)}}${{metricCard("本周总播放量",data.views)}}${{metricCard("本周篇均播放量",data.avgViews)}}</div><details><summary>导入数据</summary><div class="mini-import"><textarea id="creator-metrics-input-${{esc(c.profile_id)}}" placeholder='支持 JSON，例如 {{"opens":{{"value":12,"delta":20}},"posts":{{"value":3,"delta":-25}},"views":{{"value":50000,"delta":18}},"avgViews":{{"value":16666,"delta":8}}}}'>${{esc(JSON.stringify(data,null,2))}}</textarea><button class="primary" type="button" data-save-creator-metrics="${{esc(c.profile_id)}}">保存数据</button></div></details></section>`}}
...
function feedScriptCard(feed,c){{const entry=scriptIndex[feed.entry_id]||{{}};const ret=feedReturn(c,feed.entry_id);const image=entry.cover_url||entry.thumbnail_url||ret?.thumbnail_url||"";const share=feed.share_url||scriptShareUrl(feed.entry_id);const returnUrl=feed.return_url||ret?.video_url||"";const returnTime=feed.return_time||ret?.created_at||"";const key=feedKey({{...feed,return_url:returnUrl}});return `<article class="feed-card" data-feed-entry="${{esc(feed.entry_id)}}" data-feed-key="${{esc(key)}}"><img src="${{esc(image)}}" loading="lazy" alt=""><div><h4>${{esc(entry.title||ret?.script_title||feed.entry_id)}}</h4><p>${{esc(entry.summary||"还没有加载到脚本概述，可先保留分享链接。")}}</p><div class="share-line"><b>脚本分享链接</b><a href="${{esc(share)}}" target="_blank" rel="noopener">${{esc(share)}}</a></div></div><div class="feed-controls"><div class="feed-time-grid"><div class="feed-time-field"><label>投喂时间</label><input data-feed-date="${{esc(feed.entry_id)}}" value="${{esc(feed.date)}}"></div><div class="feed-time-field"><label>回传时间</label><input data-feed-return-time="${{esc(feed.entry_id)}}" value="${{esc(returnTime?String(returnTime).slice(0,10):"")}}" placeholder="未回传"></div></div><select data-feed-status="${{esc(feed.entry_id)}}"><option ${{feed.status==="未完成"?"selected":""}}>未完成</option><option ${{feed.status==="正在制作"?"selected":""}}>正在制作</option><option ${{feed.status==="作者不愿意拍摄"?"selected":""}}>作者不愿意拍摄</option><option ${{feed.status==="已回传"?"selected":""}}>已回传</option></select><input data-feed-return-url="${{esc(feed.entry_id)}}" value="${{esc(returnUrl)}}" placeholder="作者回传视频链接"><div class="return-preview"><img src="${{esc(ret?.thumbnail_url||image)}}" alt=""><div><b>${{returnUrl?"已回传":"未回传"}}</b><div class="small">投喂：${{esc(feed.date||"未记录")}}</div><div class="small">回传：${{esc(returnTime?timeText(returnTime):"等待回传")}}</div>${{returnUrl?`<a class="small" href="${{esc(returnUrl)}}" target="_blank" rel="noopener">打开视频</a>`:""}}</div></div><button class="danger" type="button" data-delete-feed="${{esc(key)}}" data-delete-feed-creator="${{esc(c.profile_id)}}">删除这条</button></div></article>`}}
function creatorFeedPanel(c){{const feeds=visibleCreatorFeeds(c.profile_id,[...creatorFeeds(c.profile_id),...feedsFromSubmissions(c)].filter((feed,idx,arr)=>arr.findIndex(x=>x.entry_id===feed.entry_id)===idx));return `<section class="import-panel"><h3 style="margin:0;color:#ff8200">投喂脚本及回收情况统计</h3><div id="creator-feed-stats-${{esc(c.profile_id)}}" class="feed-stats">${{feedStatsHtml(feeds,c.profile_id)}}</div><details><summary>导入/补充投喂脚本</summary><div class="mini-import"><textarea id="creator-feed-input-${{esc(c.profile_id)}}" placeholder="每行一个：2026-07-22, https://kokocomedy.com/script/..., 未完成, 回传链接（可选）"></textarea><button class="primary" type="button" data-save-creator-feeds="${{esc(c.profile_id)}}">保存投喂</button></div></details><div id="creator-feed-list-${{esc(c.profile_id)}}" class="feed-list">${{feeds.length?feeds.map(feed=>feedScriptCard(feed,c)).join(""):'<div class="empty">正在整理已有投喂和推荐脚本...</div>'}}</div></section>`}}
async function hydrateCreatorFeeds(id){{const c=creators.find(x=>x.profile_id===id);const box=document.getElementById(`creator-feed-list-${{id}}`);const stats=document.getElementById(`creator-feed-stats-${{id}}`);if(!c||!box)return;try{{await ensureScriptIndex();let feeds=[...creatorFeeds(id),...feedsFromSubmissions(c)].filter((feed,idx,arr)=>arr.findIndex(x=>x.entry_id===feed.entry_id)===idx);if(!feeds.length){{const d=await api(`/api/creator-admin/creators/${{id}}/recommendations?limit=5&offset=0`);for(const script of (d.scripts||[])){{if(script?.entry_id)scriptIndex[script.entry_id]=script;feeds.push(feedNormalize({{entry_id:script.entry_id,share_url:`__CREATOR_BASE__${{script.share_url||(`/script/${{script.entry_id}}`)}}`,date:feedDate(),status:"未完成"}}))}}feeds=feeds.filter(Boolean)}}feeds=visibleCreatorFeeds(id,feeds);if(stats)stats.innerHTML=feedStatsHtml(feeds,id);box.innerHTML=feeds.length?feeds.map(feed=>feedScriptCard(feed,c)).join(""):'<div class="empty">还没有投喂记录，也没有匹配到可推荐脚本。</div>'}}catch(err){{box.innerHTML=`<div class="empty">${{esc(err.message||err)}}</div>`}}}}
function creatorDetail(c){{const account=c.account||{{}};const type=c.creator_type||{{}};const avatar=esc(c.avatar_url||creatorFallbackAvatar());const name=c.name||c.kwai_id||"Kwai creator";return `<div class="creator-detail-top"><button type="button" data-back-creators>返回列表</button><div class="actions"><button class="primary" type="button" data-edit-creator-tags="${{esc(c.profile_id)}}">编辑标签</button><button type="button" data-refresh-creator="${{esc(c.profile_id)}}">刷新资料</button><button class="danger" type="button" data-delete-creator="${{esc(c.profile_id)}}">删除</button></div></div><section class="creator-detail-hero"><img class="avatar" src="${{avatar}}" alt=""><div><h2 style="margin:0 0 6px">${{esc(name)}}</h2><div class="small">@${{esc(c.kwai_id||"")}} · UID ${{esc(c.uid||account.uid||"未填")}} · 电话 ${{esc(c.phone||account.phone||"未填")}}</div><a class="small" href="${{esc(c.kwai_url)}}" target="_blank" rel="noopener">${{esc(c.kwai_url||"")}}</a><div class="meta"><span class="pill">POC ${{esc(creatorPocValue(c)||"未分配")}}</span><span class="pill">配合度 ${{esc(c.cooperation_level||"待标注")}}</span><span class="pill">身份 ${{esc(tagLabel(type.identity))}}</span><span class="pill">地点 ${{esc(tagLabel(type.location))}}</span>${{(c.categories||[]).map(x=>`<span class="pill">${{esc(x)}}</span>`).join("")}}</div></div><div class="submission-count"><b>${{Number(c.submission_count||0)}}</b><span>回传</span></div></section><div class="creator-detail-stack"><section class="import-panel creator-recommend-panel"><h3 style="margin:0;color:#ff8200">推荐脚本</h3><p class="small">按内容标签、身份标签和地点标签现场计算。一次展示 5 个，点击脚本卡片打开 kokocomedy 分享页。</p><button type="button" data-load-creator-scripts="${{esc(c.profile_id)}}">查看推荐脚本</button><div id="creator-scripts-${{esc(c.profile_id)}}" class="grid" data-offset="0"></div></section>${{creatorMetricsPanel(c)}}${{creatorFeedPanel(c)}}</div>`}}
function renderCreators(){{const list=document.querySelector("#creator-list");if(!list)return;const rows=filteredCreators();if(selectedCreatorId&&!creators.some(c=>c.profile_id===selectedCreatorId))selectedCreatorId="";const detail=selectedCreatorId?creators.find(c=>c.profile_id===selectedCreatorId):null;if(detail){{list.innerHTML=creatorDetail(detail);hydrateCreatorFeeds(detail.profile_id);const meta=document.querySelector("#creator-results-meta");if(meta)meta.innerHTML=`Total of <b>${{creators.length}}</b> · 当前作者：${{esc(detail.name||detail.kwai_id||detail.profile_id)}}`;const s=document.querySelector("#status");if(s)s.textContent="点击“查看推荐脚本”后才会加载推荐，避免列表卡顿。";return}}const total=rows.length;const pages=Math.max(1,Math.ceil(total/CREATOR_PAGE_SIZE));creatorPage=Math.min(Math.max(1,creatorPage),pages);const start=(creatorPage-1)*CREATOR_PAGE_SIZE;const pageRows=rows.slice(start,start+CREATOR_PAGE_SIZE);const meta=document.querySelector("#creator-results-meta");if(meta)meta.innerHTML=`Total of <b>${{total}}</b> | 所有结果(${{total}}) | kwaiId(${{rows.filter(c=>c.kwai_id).length}}) | 用户名(${{rows.filter(c=>c.name).length}})`;if(!pageRows.length){{list.innerHTML=`<div class="empty">没有匹配创作者。点击“导入创作者”添加一个 Kwai 作者主页。</div>`;return}}list.innerHTML=`<div class="creator-card-grid">${{pageRows.map(creatorTile).join("")}}</div><div class="creator-pagination"><button type="button" data-creator-page="prev" ${{creatorPage<=1?"disabled":""}}>‹</button><span class="pill">${{creatorPage}} / ${{pages}}</span><button type="button" data-creator-page="next" ${{creatorPage>=pages?"disabled":""}}>›</button><span class="small">18/page</span></div>`;const s=document.querySelector("#status");if(s)s.textContent=""}}
