...
async function loadCreators(){{try{{document.querySelector("#status")&&(document.querySelector("#status").textContent="加载注册用户、作者档案和行为数据中...");const [creatorData,accountData,analyticsPayload]=await Promise.all([api("/api/creator-admin/creators"),api("/api/creator-admin/accounts"),api(`/api/creator-admin/analytics?days=180&include_inactive=1&_=${{Date.now()}}`)]);creators=creatorData.creators||[];accounts=accountData.accounts||[];analyticsData=analyticsPayload;analyticsAutoLoaded=true;analyticsInactiveLoaded=Boolean(analyticsPayload.inactive_loaded);creatorMergeRows();await loadCreatorCloudState();activeTab="creators";adminView();const registered=accounts.filter(a=>creatorAccountIsRegistered(a)&&!analyticsIsTestUser(a)).length;document.querySelector("#status").textContent=`已合并 ${{registered}} 个注册用户和 POC 作者档案，共 ${{creatorRows.length}} 条运营对象`}}catch(e){{loginView(e.message)}}}}
...
function creatorAccountKeys(item){{const keys=new Set();const account=item?.account||{{}};for(const source of [item,account]){{for(const field of ["account_id","phone","kwai_id","uid","display_name","name"]){{const value=creatorKeyValue(source?.[field]);if(value)keys.add(value)}}for(const alias of (source?.login_aliases||[])){{const value=creatorKeyValue(alias);if(value)keys.add(value)}}}}return keys}}
...
function creatorAnalyticsUserForAccount(account,analyticsUsers){{const keys=creatorAccountKeys(account);return (analyticsUsers||[]).find(user=>[...creatorAccountKeys(user)].some(key=>keys.has(key)))||{{}}}}
function creatorMergeRows(){{const analyticsUsers=[...(analyticsData?.users||[]),...(analyticsData?.inactive_users||[])];const creatorByKey=new Map();for(const c of creators){{for(const key of creatorAccountKeys(c))if(key&&!creatorByKey.has(key))creatorByKey.set(key,c)}}const rows=[];const usedProfiles=new Set();const registeredAccounts=accounts.filter(a=>creatorAccountIsRegistered(a)&&!analyticsIsTestUser(a));for(const account of registeredAccounts){{const user=creatorAnalyticsUserForAccount(account,analyticsUsers);let profile=null;for(const key of creatorAccountKeys(account)){{if(creatorByKey.has(key)){{profile=creatorByKey.get(key);break}}}}const row={{...(profile||{{}})}};if(profile?.profile_id)usedProfiles.add(profile.profile_id);row.profile_id=profile?.profile_id||creatorPseudoId(account);row.account=account;row.account_id=account.account_id||row.account_id||"";row.phone=account.phone||row.phone||row.account_id||"";row.kwai_id=account.kwai_id||row.kwai_id||"";row.uid=account.uid||row.uid||"";row.name=profile?.name||account.display_name||account.kwai_id||account.phone||account.account_id||"未绑定作者";row.analytics_user=user;row.registration_status="registered";row.creator_source=profile?"registered_bound":"registered_phone_only";row.poc=creatorPocValue(row)||"已注册";row.submissions=Array.isArray(account.submissions)?account.submissions:(profile?.submissions||[]);rows.push(row)}}for(const c of creators){{const poc=creatorPocValue(c);if(creatorPocOptions.includes(poc)&&!usedProfiles.has(c.profile_id)){{rows.push({{...c,creator_source:"poc_profile",analytics_user:creatorAnalyticsUserForAccount(c,analyticsUsers)}})}}}creatorRows=rows;return rows}}
...
function creatorSubmissionCount(c){{const accountCount=Number(c?.account?.submission_count||c?.analytics_user?.submission_count||c?.submission_count||0);const feedCount=creatorFeedSummary(c).returnCount;return Math.max(accountCount,feedCount)}}
...
function creatorSortedRows(rows){{return [...rows].sort((a,b)=>{{const ar=a.registration_status==="registered"?1:0;const br=b.registration_status==="registered"?1:0;return (br-ar)||(creatorSortTime(b)-creatorSortTime(a))||(creatorSubmissionCount(b)-creatorSubmissionCount(a))||(creatorOpenedScriptCount(b)-creatorOpenedScriptCount(a))||String(a.name||a.kwai_id||"").localeCompare(String(b.name||b.kwai_id||""),"zh-CN")}})}}
...
function creatorTile(c){{const account=c.account||{{}};const avatar=esc(c.avatar_url||creatorFallbackAvatar());const name=c.name||c.kwai_id||"Kwai creator";const kwai=c.kwai_id||"-";const uid=c.uid||account.uid||"-";const phone=c.phone||account.phone||"-";const poc=creatorPocValue(c)||"未分配";const source=c.creator_source==="registered_phone_only"?"待绑定 Kwai":(c.registration_status==="registered"?"已注册":"POC 档案");return `<article class="creator-tile" data-open-creator="${{esc(c.profile_id)}}"><div class="creator-tile-head"><img class="avatar" src="${{avatar}}" alt=""><div><h3 class="creator-tile-name">${{esc(name)}}</h3><div class="small">Kwai ID&nbsp; ${{esc(kwai)}}</div></div></div><div class="meta"><span class="creator-badge">${{esc(source)}}</span><span class="pill">POC ${{esc(poc)}}</span>${{(c.categories||[]).slice(0,2).map(x=>`<span class="pill">${{esc(x)}}</span>`).join("")}}</div><div class="creator-field-lines"><div class="creator-field-line"><span>UID</span><span>${{esc(uid)}}</span></div><div class="creator-field-line"><span>电话</span><span>${{esc(phone)}}</span></div></div><div class="creator-stats-row"><div class="creator-stat"><span>打开 Koko</span><b>${{creatorOpenCount(c)}}</b></div><div class="creator-stat"><span>打开脚本</span><b>${{creatorOpenedScriptCount(c)}}</b></div><div class="creator-stat"><span>回传</span><b>${{creatorSubmissionCount(c)}}</b></div></div></article>`}}
function creatorRow(c){{const type=c.creator_type||{{}};const avatar=esc(c.avatar_url||creatorFallbackAvatar());const name=c.name||c.kwai_id||"Kwai creator";const kwai=c.kwai_id||"-";const poc=creatorPocValue(c)||"未分配";const tags=[c.creator_source==="registered_phone_only"?"待绑定 Kwai":(c.registration_status==="registered"?"已注册":"POC 档案"),`POC ${{poc}}`,tagLabel(type.identity),tagLabel(type.location),...(c.categories||[])].filter(Boolean);return `<article class="creator-row" data-open-creator="${{esc(c.profile_id)}}"><div class="creator-row-main"><img class="avatar" src="${{avatar}}" alt=""><div class="creator-row-title"><b>${{esc(name)}}</b><span class="small">@${{esc(kwai)}} · 电话 ${{esc(c.phone||c.account?.phone||"-")}}</span></div></div><div class="creator-row-num" data-label="打开 Koko">${{creatorOpenCount(c)}}</div><div class="creator-row-num" data-label="打开脚本">${{creatorOpenedScriptCount(c)}}</div><div class="creator-row-num" data-label="回传">${{creatorSubmissionCount(c)}}</div><div class="creator-row-tags">${{tags.slice(0,8).map(x=>`<span class="pill">${{esc(x)}}</span>`).join("")}}</div></article>`}}
...
function creatorMetricsPanel(c){{const user=c.analytics_user||{{}};const totalDuration=Number(user.platform_duration_seconds||0)+Number(user.script_duration_seconds||0);const submissionCount=creatorSubmissionCount(c);return `<section class="import-panel"><div class="feed-stats-head"><h3 style="margin:0;color:#ff8200">作者使用数据</h3><button type="button" data-copy="${{esc(c.phone||c.account_id||"")}}">复制账号</button></div><div class="metrics-grid">${{analyticsSummaryCard("打开 Koko 次数",creatorOpenCount(c),"只统计 Creator 首页/平台打开")}}${{analyticsSummaryCard("打开脚本个数",creatorOpenedScriptCount(c),"按看过的脚本去重，缺少明细时用打开数兜底")}}${{analyticsSummaryCard("回传数",submissionCount,"已合并运营投喂回收和账号真实回传")}}${{analyticsSummaryCard("总停留",secondsText(totalDuration),"首页 + 脚本页")}}</div>${{analyticsAuthorDetail(user,submissionCount)}}</section>`}}
