...
function creatorTile(c){{const account=c.account||{{}};const type=c.creator_type||{{}};const avatar=esc(c.avatar_url||creatorFallbackAvatar());const name=c.name||c.kwai_id||"Kwai creator";const kwai=c.kwai_id||"-";const uid=c.uid||account.uid||"-";const phone=c.phone||account.phone||"-";const poc=creatorPocValue(c)||"未分配";const fans=creatorMetric(c,["followers","follower_count","fans"],"-");const posts=creatorMetric(c,["post_count","video_count","works_count","total_posts"],"-");return `<article class="creator-tile" data-open-creator="${{esc(c.profile_id)}}"><div class="creator-tile-head"><img class="avatar" src="${{avatar}}" alt=""><div><h3 class="creator-tile-name">${{esc(name)}}</h3><div class="small">Kwai ID&nbsp; ${{esc(kwai)}}</div></div></div><div class="meta"><span class="creator-badge">POC ${{esc(poc)}}</span><span class="pill">${{esc(c.cooperation_level||"Normal")}}</span>${{(c.categories||[]).slice(0,2).map(x=>`<span class="pill">${{esc(x)}}</span>`).join("")}}</div><div class="creator-field-lines"><div class="creator-field-line"><span>UID</span><span>${{esc(uid)}}</span></div><div class="creator-field-line"><span>电话</span><span>${{esc(phone)}}</span></div></div><div class="creator-stats-row"><div class="creator-stat"><span>粉丝数</span><b>${{esc(fans)}}</b></div><div class="creator-stat"><span>总发文数</span><b>${{esc(posts)}}</b></div></div><div class="small" style="margin-top:10px">身份：${{esc(type.identity||"待标注")}} · 地点：${{esc(type.location||"待标注")}} · 回传 ${{Number(c.submission_count||0)}}</div></article>`}}
