...
function analyticsCoreTimelineHtml(){{const rows=(analyticsData?.timeline?.hourly||[]).slice().sort((a,b)=>String(b.hour||"").localeCompare(String(a.hour||"")));if(!rows.length)return `<details class="analytics-core-detail"><summary>展开小时明细 <small>当前接口还没有返回小时数据</small></summary><div class="empty" style="margin-top:12px">暂无可展示的小时明细。等待 kokocomedy 数据接口更新后会自动显示。</div></details>`;return `<details class="analytics-core-detail"><summary>展开小时明细 <small>${{rows.length}} 个小时段</small></summary><div class="analytics-hour-table"><div class="analytics-hour-head"><span>时间</span><span>注册</span><span>直接打开</span><span>分享链接打开</span><span>脚本打开</span><span>平均时长</span></div>${{rows.map(row=>`<div class="analytics-hour-row"><span data-label="时间">${{esc(analyticsHourLabel(row.hour))}}</span><b data-label="注册">${{Number(row.registered_users||0)}}</b><b data-label="直接打开">${{Number(row.platform_opens||0)}}</b><b data-label="分享链接打开">${{Number(row.share_link_opens||0)}}</b><b data-label="脚本打开">${{Number(row.script_opens||0)}}</b><span data-label="平均时长">${{secondsText(row.avg_script_duration_seconds||0)}}</span></div>`).join("")}}</div></details>`}}
