...
function analyticsDailySummaryHtml(hourRows,users){{const map=new Map();const ensure=day=>{{if(!map.has(day))map.set(day,{{users:new Set(),durationSum:0,durationCount:0,submissions:0}});return map.get(day)}};for(const row of hourRows){{const day=analyticsDayKey(row.hour);const bucket=ensure(day);for(const item of [...(row.platform_open_details||[]),...(row.script_open_details||[]),...(row.duration_details||[])]){{if(analyticsIsTestUser(item))continue;const key=analyticsEntityKey(item);if(key)bucket.users.add(key)}}for(const item of (row.duration_details||[])){{if(analyticsIsTestUser(item))continue;const duration=analyticsValidStaySeconds(item.duration_seconds);if(duration){{bucket.durationSum+=duration;bucket.durationCount+=1}}}}}}for(const user of (users||[])){{if(analyticsIsTestUser(user))continue;for(const sub of (user.submissions||[])){{const day=analyticsDayKey(sub.created_at);ensure(day).submissions+=1}}}}const days=[...map.entries()].sort((a,b)=>String(b[0]).localeCompare(String(a[0])));if(!days.length)return `<section class="import-panel"><h3 style="margin:0;color:#1f1f1f">每日数据总结</h3><div class="empty">暂无每日数据。</div></section>`;return `<section class="import-panel"><h3 style="margin:0;color:#1f1f1f">每日数据总结</h3><div class="analytics-hour-table"><div class="analytics-hour-head" style="grid-template-columns:150px 1fr 1fr 1fr"><span>日期</span><span>Koko 使用人数</span><span>平均停留时间</span><span>脚本回传数据</span></div>${{days.map(([day,bucket])=>`<div class="analytics-hour-row" style="grid-template-columns:150px 1fr 1fr 1fr"><span data-label="日期">${{esc(day.replaceAll("-","."))}}</span><b data-label="Koko 使用人数">${{bucket.users.size}}</b><span data-label="平均停留时间">${{bucket.durationCount?secondsText(bucket.durationSum/bucket.durationCount):"-"}}</span><b data-label="脚本回传数据">${{bucket.submissions}}</b></div>`).join("")}}</div><small class="small">平均停留已排除测试账号 666、0 秒和超过 30 分钟的异常停留。</small></section>`}}
