...
function analyticsInactiveRows(rows){{if(!rows.length)return "";return `<details class="import-panel" style="margin-top:14px"><summary>无行为账号（${{rows.length}} 个，默认折叠）</summary><div class="creator-row-list" style="margin-top:12px">${{rows.map(user=>`<article class="analytics-row"><div class="creator-row-main"><div class="avatar"></div><div class="creator-row-title"><b>${{esc(user.display_name||user.account_id||user.phone)}}</b><span class="small">${{esc(user.phone||user.account_id)}} · 预置时间：${{esc(timeText(user.provisioned_at||user.created_at))}}</span></div></div><div>${{analyticsStatusPill(user)}}</div><div class="small">暂无打开、点击或回传行为</div></article>`).join("")}}</div></details>`}}
function renderAnalytics(){{const box=document.querySelector("#analytics-board");if(!box)return;if(!analyticsData){{box.innerHTML=`<div class="empty">页面已经打开。点击“刷新数据”后会先展示有行为的用户，没有行为的账号默认折叠。</div>`;return}}const s=analyticsData.summary||{{}};const users=analyticsData.users||[];const inactive=analyticsData.inactive_users||[];const activeHtml=users.length?`<section class="creator-row-list"><div class="analytics-row-head"><span>用户</span><span>状态</span><span>平台打开</span><span>分享页打开</span><span>平台停留</span><span>脚本停留</span><span>点击/回传</span></div>${{users.map(user=>`<article class="analytics-row"><div class="creator-row-main"><div class="avatar"></div><div class="creator-row-title"><b>${{esc(user.display_name||user.account_id||user.phone)}}</b><span class="small">${{esc(user.phone||user.account_id)}} · 注册时间：${{esc(timeText(user.registered_at||user.provisioned_at||user.created_at))}}</span></div></div><div>${{analyticsStatusPill(user)}}</div><div class="creator-row-num" data-label="平台打开">${{Number(user.platform_open_count||0)}}</div><div class="creator-row-num" data-label="分享页打开">${{Number(user.script_share_open_count||0)}}</div><div>${{secondsText(user.platform_duration_seconds||0)}}</div><div>${{secondsText(user.script_duration_seconds||0)}}</div><div class="analytics-mini">${{analyticsClickPills(user)}}<span class="pill">回传 ${{Number(user.submission_count||0)}}</span></div><details><summary>查看脚本浏览、点击和回传明细</summary><div class="creator-detail-grid"><section><h3>脚本浏览</h3>${{analyticsScriptRows(user)}}</section><section><h3>回传记录</h3>${{analyticsSubmissionRows(user)}}</section></div><div class="small" style="margin-top:10px">最近行为：${{(user.recent_events||[]).slice(0,8).map(e=>`${{esc(timeText(e.created_at))}} / ${{esc(e.event)}} / ${{esc(e.path||"")}}`).join("<br>")||"暂无"}}</div></details></article>`).join("")}}</section>`:`<div class="empty">最近 ${{analyticsData.days||30}} 天还没有用户行为。</div>`;box.innerHTML=`<section class="analytics-summary">${{analyticsSummaryCard("账号数",s.accounts||0)}}${{analyticsSummaryCard("有行为账号",s.active_accounts||users.length)}}${{analyticsSummaryCard("无行为账号",s.inactive_accounts||inactive.length,"默认折叠")}}${{analyticsSummaryCard("平台打开",s.platform_opens||0)}}${{analyticsSummaryCard("分享页打开",s.script_opens||0)}}${{analyticsSummaryCard("回传",s.submissions||0)}}</section>${{activeHtml}}${{analyticsInactiveRows(inactive)}}`;const status=document.querySelector("#status");if(status)status.textContent=`最近 ${{analyticsData.days||30}} 天：显示 ${{users.length}} 个有行为账号，折叠 ${{inactive.length}} 个无行为账号，${{s.events||0}} 条行为事件。`}}
async function loadAnalytics(){{activeTab="analytics";if(!document.querySelector("#analytics-board"))adminView();const status=document.querySelector("#status");const box=document.querySelector("#analytics-board");if(status)status.textContent="正在刷新 kokocomedy 数据，不会阻塞页面...";if(box)box.innerHTML=`<div class="empty">正在拉取数据，请稍等。你可以先停留在这个页面，不会白屏。</div>`;try{{analyticsData=await api("/api/creator-admin/analytics?days=30");renderAnalytics()}}catch(e){{if(status)status.textContent=e.message||"数据刷新失败";if(box)box.innerHTML=`<div class="empty">数据刷新失败：${{esc(e.message||"请求超时或 Creator 服务暂时不可用")}}<br><br><button class="primary" id="retry-analytics" type="button">重新刷新</button></div>`;document.querySelector("#retry-analytics")?.addEventListener("click",loadAnalytics)}}}}
...
async function loadCurrentTab(){{if(activeTab==="creators")return loadCreators();if(activeTab==="analytics"){{adminView();return}}if(activeTab==="accounts")return loadAccounts();if(activeTab==="submissions")return loadSubmissions();if(activeTab==="intakes")return loadIntakes();if(activeTab==="imports"){{adminView();return}}return loadEntries()}}
