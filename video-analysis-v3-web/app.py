...
function creatorSubmissionCount(c){{const rawCount=creatorRawSubmissionCount(c);const fallbackCount=Math.max(Number(c?.account?.submission_count||0),Number(c?.analytics_user?.submission_count||0),Number(c?.submission_count||0));const accountCount=rawCount||fallbackCount;const feedCount=creatorFeedSummary(c).returnCount;return Math.max(accountCount,feedCount)}}
...
function creatorSortedRows(rows){{return [...rows].sort((a,b)=>(creatorSubmissionCount(b)-creatorSubmissionCount(a))||(creatorFeedCount(b)-creatorFeedCount(a))||(creatorSortTime(b)-creatorSortTime(a))||(creatorOpenedScriptCount(b)-creatorOpenedScriptCount(a))||String(a.name||a.kwai_id||"").localeCompare(String(b.name||b.kwai_id||""),"zh-CN"))}}
...
function creatorTile(c){{const account=c.account||{{}};const avatar=esc(c.avatar_url||creatorFallbackAvatar());const name=c.name||c.kwai_id||"Kwai creator";const kwai=c.kwai_id||"-";const uid=c.uid||account.uid||"-";const phone=c.phone||account.phone||"-";const poc=creatorPocValue(c)||"未分配";const source=c.creator_source==="registered_phone_only"?"待绑定 Kwai":(c.registration_status==="registered"?"已注册":"POC 档案");return `<article class="creator-tile" data-open-creator="${{esc(c.profile_id)}}"><div class="creator-tile-head"><img class="avatar" src="${{avatar}}" alt=""><div><h3 class="creator-tile-name">${{esc(name)}}</h3><div class="small">Kwai ID&nbsp; ${{esc(kwai)}}</div></div></div><div class="meta"><span class="creator-badge">${{esc(source)}}</span><span class="pill">POC ${{esc(poc)}}</span>${{(c.categories||[]).slice(0,2).map(x=>`<span class="pill">${{esc(x)}}</span>`).join("")}}</div><div class="creator-field-lines"><div class="creator-field-line"><span>UID</span><span>${{esc(uid)}}</span></div><div class="creator-field-line"><span>电话</span><span>${{esc(phone)}}</span></div></div><div class="creator-stats-row"><div class="creator-stat"><span>投喂脚本</span><b>${{creatorFeedCount(c)}}</b></div><div class="creator-stat"><span>回传脚本</span><b>${{creatorSubmissionCount(c)}}</b></div><div class="creator-stat"><span>打开 Koko</span><b>${{creatorOpenCount(c)}}</b></div><div class="creator-stat"><span>打开脚本</span><b>${{creatorOpenedScriptCount(c)}}</b></div></div></article>`}}
function creatorRow(c){{const type=c.creator_type||{{}};const avatar=esc(c.avatar_url||creatorFallbackAvatar());const name=c.name||c.kwai_id||"Kwai creator";const kwai=c.kwai_id||"-";const poc=creatorPocValue(c)||"未分配";const tags=[c.creator_source==="registered_phone_only"?"待绑定 Kwai":(c.registration_status==="registered"?"已注册":"POC 档案"),`POC ${{poc}}`,tagLabel(type.identity),tagLabel(type.location),...(c.categories||[])].filter(Boolean);return `<article class="creator-row" data-open-creator="${{esc(c.profile_id)}}"><div class="creator-row-main"><img class="avatar" src="${{avatar}}" alt=""><div class="creator-row-title"><b>${{esc(name)}}</b><span class="small">@${{esc(kwai)}} · 电话 ${{esc(c.phone||c.account?.phone||"-")}}</span></div></div><div class="creator-row-num" data-label="投喂">${{creatorFeedCount(c)}}</div><div class="creator-row-num" data-label="回传">${{creatorSubmissionCount(c)}}</div><div class="creator-row-num" data-label="打开 Koko">${{creatorOpenCount(c)}}</div><div class="creator-row-num" data-label="打开脚本">${{creatorOpenedScriptCount(c)}}</div><div class="creator-row-tags">${{tags.slice(0,8).map(x=>`<span class="pill">${{esc(x)}}</span>`).join("")}}</div></article>`}}
