...
function answerText(answer){{if(!answer)return "未填写";const base=answer.label_zh||answer.label_pt||answer.option_id||"未填写";return answer.option_id==="other"&&answer.other_text?`${{base}}：${{answer.other_text}}`:base}}
function filteredIntakes(){{const q=String(document.querySelector("#intake-search")?.value||"").trim().toLowerCase();if(!q)return intakes;return intakes.filter(item=>[item.kwai_name,item.kwai_url,item.whatsapp,item.notes,JSON.stringify(item.answers||{{}})].join(" ").toLowerCase().includes(q))}}
function renderIntakes(){{const list=document.querySelector("#intake-list");if(!list)return;const rows=filteredIntakes();if(!rows.length){{list.innerHTML=`<div class="empty">还没有作者信息提交。公开问卷地址：<br><a class="small" href="__CREATOR_BASE__/creator-survey" target="_blank" rel="noopener">__CREATOR_BASE__/creator-survey</a></div>`;return}}list.innerHTML=`<section class="submission-summary"><div><h2>作者信息收集</h2><p class="copy">公开问卷地址：<a href="__CREATOR_BASE__/creator-survey" target="_blank" rel="noopener">__CREATOR_BASE__/creator-survey</a></p></div><div class="submission-count"><b>${{rows.length}}</b><span>条信息</span></div></section>`+rows.map(item=>{{const a=item.answers||{{}};return `<article class="creator-card"><div class="creator-head"><div class="avatar"></div><div><h3 class="creator-name">${{esc(item.kwai_name||"未命名作者")}}</h3><div class="small">提交时间：${{esc(timeText(item.created_at))}}</div><div class="meta"><span class="pill">人数：${{esc(answerText(a.people))}}</span><span class="pill">场景：${{esc(answerText(a.scene))}}</span><span class="pill">笑点：${{esc(answerText(a.humor))}}</span></div>${{item.kwai_url?`<a class="small" href="${{esc(item.kwai_url)}}" target="_blank" rel="noopener">${{esc(item.kwai_url)}}</a>`:""}}</div><div class="actions"><button type="button" data-copy="${{esc(item.kwai_name||"")}}">复制名称</button>${{item.kwai_url?`<button type="button" data-copy="${{esc(item.kwai_url)}}">复制主页</button>`:""}}</div></div><div class="script-mini"><div></div><div><b>联系方式 / 备注</b><span>${{esc(item.whatsapp||"未填写联系方式")}}</span><span>${{esc(item.notes||"无备注")}}</span></div><button type="button" data-copy="${{esc(JSON.stringify(item))}}">复制整条</button></div></article>`}}).join("")}}
