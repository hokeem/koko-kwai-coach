...
function analyticsAuthorDetail(user){{const totalDuration=Number(user.platform_duration_seconds||0)+Number(user.script_duration_seconds||0);const scriptOpens=Number(user.script_share_open_count||0);const avgScript=scriptOpens?Number(user.script_duration_seconds||0)/scriptOpens:0;return `<details class="analytics-user-detail" style="grid-column:1/-1;margin-top:8px;padding:12px;border:1px solid rgba(255,130,0,.14);border-radius:16px;background:#fffaf5"><summary>展开这个作者的停留、点击、脚本浏览和回传</summary><div class="analytics-summary" style="grid-template-columns:repeat(4,minmax(0,1fr));margin-top:12px"><div class="metric-card"><span>Koko 总停留</span><b>${{secondsText(totalDuration)}}</b><small class="small">首页 ${{secondsText(user.platform_duration_seconds)}} · 脚本 ${{secondsText(user.script_duration_seconds)}}</small></div><div class="metric-card"><span>打开次数</span><b>${{Number(user.platform_open_count||0)+scriptOpens}}</b><small class="small">首页 ${{Number(user.platform_open_count||0)}} · 脚本 ${{scriptOpens}}</small></div><div class="metric-card"><span>平均脚本停留</span><b>${{secondsText(avgScript)}}</b><small class="small">按脚本打开次数计算</small></div><div class="metric-card"><span>回传脚本</span><b>${{Number(user.submission_count||0)}}</b><small class="small">已按手机号/Kwai/回传链接匹配</small></div></div><section class="import-panel"><h3 style="margin:0;color:#1f1f1f">点击组件</h3><div class="meta">${{analyticsClickPills(user)}}</div></section><section class="import-panel"><h3 style="margin:0;color:#1f1f1f">看过的脚本</h3>${{analyticsScriptRows(user)}}</section><section class="import-panel"><h3 style="margin:0;color:#1f1f1f">回传记录</h3>${{analyticsSubmissionRows(user)}}</section><section class="import-panel"><h3 style="margin:0;color:#1f1f1f">最近行为流水</h3>${{analyticsRecentEventRows(user)}}</section></details>`}}
...
async function loadAnalytics(includeInactive=false){{activeTab="analytics";if(!document.querySelector("#analytics-board"))adminView();const status=document.querySelector("#status");const box=document.querySelector("#analytics-board");if(analyticsLoading)return;analyticsLoading=true;if(status)status.textContent=includeInactive?"正在加载无行为账号...":"正在加载有行为用户...";if(box&&!analyticsData)box.innerHTML=`<div class="empty">正在拉取有行为用户，不会等待无行为账号。</div>`;try{{const data=await api(`/api/creator-admin/analytics?days=180&include_inactive=${{includeInactive?"1":"0"}}&_=${{Date.now()}}`);if(includeInactive&&analyticsData){{analyticsData={{...analyticsData,summary:data.summary||analyticsData.summary,inactive_users:data.inactive_users||[],inactive_loaded:true}};analyticsInactiveLoaded=true}}else{{analyticsData=data;analyticsInactiveLoaded=Boolean(data.inactive_loaded)}}analyticsAutoLoaded=true;renderAnalytics()}}catch(e){{const msg=e.message||"Creator 数据接口暂时不可用";if(status)status.textContent=`数据加载失败：${{msg}}`;if(box&&!analyticsData)box.innerHTML=`<div class="empty">数据加载失败：${{esc(msg)}}<br><br><button class="primary" id="retry-analytics" type="button">重新加载有行为用户</button></div>`;document.querySelector("#retry-analytics")?.addEventListener("click",()=>loadAnalytics(false))}}finally{{analyticsLoading=false}}}}
