...
async function loadSubmissions(append=false){{if(submissionsLoading)return;try{{submissionsLoading=true;const status=document.querySelector("#status");if(status)status.textContent=append?"加载更多回传中...":"加载最近回传数据中...";const offset=append?submissionsOffset:0;const d=await api(`/api/creator-admin/submissions?limit=${{SUBMISSIONS_PAGE_SIZE}}&offset=${{offset}}&_=${{Date.now()}}`);const incoming=d.submissions||[];submissions=append?[...submissions,...incoming]:incoming;submissionsTotal=Number(d.total||submissions.length||0);submissionsOffset=submissions.length;activeTab="submissions";adminView();const nextStatus=document.querySelector("#status");if(nextStatus)nextStatus.textContent=`已加载 ${{submissions.length}} / ${{submissionsTotal}} 条回传`}}catch(e){{loginView(e.message)}}finally{{submissionsLoading=false;renderSubmissionStats()}}}}
