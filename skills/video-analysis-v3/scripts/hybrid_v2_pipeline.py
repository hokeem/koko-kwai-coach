...
硬性要求：
- 必须覆盖整条视频，而不是只写前面一小段
- 最终结果是给 Koko Creator 入库和翻译用的脚本包，不是分析报告。所有面向用户的文字必须短、直、可拍摄。
- `whole_video_summary` 必须是一段简洁剧情概述，按“起因 -> 推进 -> 结果/包袱”写，建议 60-120 个中文字符。不要写“揭示复杂关系/反映人性/社会判断”等抽象议论文。
- `dialogue_or_audio` 必须是中文直译，只翻译，不改写，不润色，不换说法
- `core_viral_points` 要保留 1-3 项，每一项只写一句短爆点，直接说明视频为什么抓人、反差点在哪，不要写成长段分析。
- `replaceable_parts` 必须给出 1-3 个可直接套用的替换元素，不是建议；优先用 `人物关系`、`冲突事项`、`场景`、`道具/诱因`、`结尾反转` 这类标签。
- `rows` 必须写成脚本库分镜表：5-8 个剧情/动作 beat。`visual_content` 只写拍摄准备信息（地点/场景、在场人物、必要道具），目标 24 个中文字符以内；`action` 写具体调度/表情/动作推进，保持紧凑；`dialogue_or_audio` 按说话人分行
...
请输出严格 JSON，字段如下：
{
  ...
  "source_url": "原视频链接",
  "whole_video_summary": "一段简洁剧情概述：起因 -> 推进 -> 结果/包袱，不写抽象议论文",
  "core_viral_points": [
...
成品写法 SOP（必须遵守）：
- 最终结果是给 Creator 平台展示、翻译、同步入库用的脚本包，不是给内部看的分析报告。能短就短，能具体就具体。
...
- `whole_video_summary` 要像给创作者看的“剧情概述”，重点写：起因 -> 推进 -> 关键证据/对质 -> 最终落点。建议 60-120 个中文字符。总结必须落在具体事件和最终结果上，而不是抽象拔高成“揭示了复杂关系/社会判断/人性弱点”这类空泛句。
...
- `core_viral_points` 重新保留 1-3 项，但必须是短句式爆点，不要写成长报告。
- `replaceable_parts` 必须是 1-3 个可直接点击套用的替换元素，不能只写“可以替换场景/人物”这种建议。
...
- `rows` 必须写成脚本库分镜表，不是观察报告：
  ...
  - `visual_content` 只写拍摄准备信息：地点/场景、在场人物、必要道具。目标 24 个中文字符以内，例如“厨房内；丈夫、妻子；碗、抹布”。不要写镜头分析、完整动作链、对话内容、情绪变化或剧情总结。
  - `action` 写导演分镜式动作：谁在画面哪里做什么、表情/情绪如何变化、这个动作如何推动冲突或笑点。每格 1-2 句，避免长段。
...
最终输出必须是严格的 v2 `script_table.json` 结构：
{
  ...
  "core_viral_points": [
    {"label": "冲突钩子", "text": "人物一开场就进入不对劲的状态，观众会马上想知道原因。"},
    {"label": "结局反转", "text": "前面把观众往严重方向带，最后用生活化真相翻回来。"}
  ],
  "replaceable_parts": [
    {"label": "冲突事项", "text": "把原冲突替换成同强度、同关系压力的新任务或误会。"}
  ],
  "rows": [
    {
      "source_url": "原视频链接",
      "time": "00:00-00:15",
      "visual_content": "厨房内；丈夫、妻子；碗、抹布",
      "action": "妻子擦碗，丈夫慢慢靠近，小心试探提出请求。",
...
- 不要从一句模糊台词直接推出整条故事主轴
- `whole_video_summary` 要明确写出起因、经过、结果，重点是故事是否讲通；保持短、直、可给创作者阅读
- `dialogue_or_audio` 继续保持中文忠实翻译
- `rows` 继续按脚本库分镜表写：`visual_content` 是拍摄准备信息（地点/场景、在场人物、必要道具，目标 24 个中文字符以内），`action` 是人物调度、表情和动作推进，`dialogue_or_audio` 按说话人分行
...
输出严格 JSON，结构与 script_table.json 保持兼容，但额外补充：
{
  ...
  "audio_information_score": "0/10 到 10/10",
  "source_url": "原视频链接",
  "whole_video_summary": "按起因-经过-结果写出的简洁候选总结",
  "core_viral_points": [],
